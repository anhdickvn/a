...
private func decodeHoverItemURL(_ url: URL) -> MCChatHoverItem? {
    ...
    return MCChatHoverItem(resourceName: resourceName, displayName: displayName, count: count, legacyItemId: legacy, damage: damage, lore: [], rawValue: object["rawValue"] as? String)
}
...
private final class StableMovementPad: UIView {
    ...
    private func setup() {
        ...
        let w = makeMoveButton("W", 1, 0)
        topRow.addArrangedSubview(spacer())
        topRow.addArrangedSubview(w)
        topRow.addArrangedSubview(spacer())

        bottomRow.addArrangedSubview(makeMoveButton("A", 0, -1))
        bottomRow.addArrangedSubview(makeMoveButton("S", -1, 0))
        bottomRow.addArrangedSubview(makeMoveButton("D", 0, 1))

        style(spaceButton, title: "SPACE", size: 18)
        spaceButton.widthAnchor.constraint(equalToConstant: 128).isActive = true
        spaceButton.heightAnchor.constraint(equalToConstant: 56).isActive = true
        spaceButton.addTarget(self, action: #selector(spaceDown), for: .touchDown)
        spaceButton.addTarget(self, action: #selector(spaceUp), for: [.touchUpInside, .touchUpOutside, .touchCancel])
    ...
    private func style(_ button: UIButton, title: String, size: CGFloat) {
        button.setTitle(title, for: .normal)
        button.setTitleColor(.white, for: .normal)
        button.titleLabel?.font = .systemFont(ofSize: size, weight: .bold)
        button.backgroundColor = UIColor.white.withAlphaComponent(0.12)
    ...
    private func makeMoveButton(_ title: String, _ forward: Double, _ strafe: Double) -> UIButton {
        ...
        // Chỉ dùng touchUpInside/touchUpOutside/touchCancel để coi là "nhả nút".
        // KHÔNG dùng touchDragExit: nút 72x72pt khá nhỏ, ngón tay chỉ cần run nhẹ
        // ra khỏi biên là touchDragExit bắn liên tục -> di chuyển bị bật/tắt chớp
        // nhoáng dù người dùng vẫn đang giữ tay trên màn hình (đây là nguyên nhân
        // chính khiến WASD "chạy log nhưng không đi được mượt").
        button.addTarget(self, action: #selector(moveDown(_:)), for: .touchDown)
        button.addTarget(self, action: #selector(moveUp(_:)), for: [.touchUpInside, .touchUpOutside, .touchCancel])
        return button
    }
    ...
    @objc private func moveDown(_ sender: UIButton) {
        ...
        // MCClient.startMoving() đã tự khởi động một vòng lặp 20Hz riêng
        // (movementControlTask) để bơm packet trong lúc giữ nút. Trước đây
        // StableMovementPad còn tạo THÊM một Timer 0.05s nữa cũng gọi
        // movePlayer() -> hai vòng lặp cùng đẩy tọa độ song song, gửi packet
        // dồn dập và không đều, dễ bị server/anti-cheat coi là di chuyển bất
        // thường rồi bẻ vị trí về chỗ cũ. Giờ chỉ để MCClient tự quản lý vòng
        // lặp duy nhất; nút chỉ báo bắt đầu/kết thúc.
