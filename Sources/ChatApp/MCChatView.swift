...
struct MCChatView: View {
    ...
    var body: some View {
        // Disable long-press text selection/copy like ChatCraft chat.
        let _ = UITextField.appearance()
        ZStack {
        ...
        .onChange(of: input) { _ in
            // Chỉ tìm/gợi ý tên player khi người dùng thực sự nhấn TAB.
            // Khi gõ bình thường (ví dụ "xin chào izumin") tuyệt đối không hiện danh sách tên.
    ...
    private var chatView: some View {
        VStack(spacing: 0) {
            ...
            // Toàn bộ transcript nằm trong MỘT UITextView duy nhất nhưng cố ý vô hiệu hóa
            // selection/copy/long-press. Người dùng chỉ được vuốt dọc để đọc chat; các URL
            // và item hover vẫn được bắt bằng tap gesture riêng.
                ...
                HStack(spacing: 10) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        if let segments = item.nameSegments, !segments.isEmpty {
                            coloredText(segments).font(.system(size: 14, weight: .regular))
                        } else {
                            Text(item.plainName)
                                .font(.system(size: 14, weight: .regular))
                                .foregroundStyle(.blue)
                ...
                ForEach(Array(slots), id: \.self) { slot in
                    if let item = client.playerInventory[slot] {
                        HStack(spacing: 10) {
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                if let segments = item.nameSegments, !segments.isEmpty {
                                    coloredText(segments).font(.system(size: 14))
                                } else {
...
private final class MCNonSelectableChatTextView: UITextView {
...
private struct ChatTranscriptTextView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UITextView {
        let view = MCNonSelectableChatTextView()
    ...
    private func makeTranscriptAttributedString() -> NSAttributedString {
        ...
        for (index, entry) in entries.enumerated() {
            let message = (entry.segments?.isEmpty == false) ? attributedMessage(entry.segments!, underlinePlayerChat: entry.kind == .chat) : NSAttributedString(string: entry.text, attributes: [
    ...
    private func attributedMessage(_ segments: [MCChatSegment], underlinePlayerChat: Bool = false) -> NSAttributedString {
        let result = NSMutableAttributedString()
        for segment in segments {
            var traits: UIFontDescriptor.SymbolicTraits = []
            if segment.bold { traits.insert(.traitBold) }
            if segment.italic { traits.insert(.traitItalic) }
            let descriptor = UIFont.systemFont(ofSize: 14).fontDescriptor.withSymbolicTraits(traits) ?? UIFont.systemFont(ofSize: 14).fontDescriptor
            var attrs: [NSAttributedString.Key: Any] = [
                .font: UIFont(descriptor: descriptor, size: 14),
                .foregroundColor: UIColor(segment.colorHex.flatMap(Color.init(hex:)) ?? .white)
            ]
            if false { attrs[.strikethroughStyle] = NSUnderlineStyle.single.rawValue }
            // underline disabled
...
if segment.strikethrough { attributes[.strikethroughStyle] = NSUnderlineStyle.single.rawValue }
            if let linkString = segment.linkURL, let link = URL(string: linkString) {
                attributes[.mcInteractive] = link} else if let hover = segment.hoverItem, let encoded = encodeHoverItemURL(hover) {
        ...
        // Tự nhận diện URL trong chat để người dùng có thể click, không gạch chân.
        if let detector = try? NSDataDetector(types: NSTextCheckingResult.CheckingType.link.rawValue) {
            let range = NSRange(location: 0, length: (fullText as NSString).length)
            detector.enumerateMatches(in: fullText, options: [], range: range) { match, _, _ in
                guard let url = match?.url, let matchRange = match?.range else { return }
...
private final class StableMovementPad: UIView {
    ...
    private func setup() {
        ...
        style(spaceButton, title: "Jump", size: 14)
