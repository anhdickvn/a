...
private final class StableMovementPad: UIView {
    ...
    @objc private func moveDown(_ sender: UIButton) {
        ...
        repeatTimers[sender]?.invalidate()
        let timer = Timer(timeInterval: 0.05, repeats: true) { [weak self, weak sender] _ in
            guard let self, let sender, self.activeIDs.contains(ObjectIdentifier(sender)), let client = self.client, let vector = self.buttons[sender] else { return }
            client.movePlayer(forward: vector.0, strafe: vector.1)
        }
        repeatTimers[sender] = timer
        RunLoop.main.add(timer, forMode: .common)
    }
