...
struct MCChatView: View {
    ...
    var body: some View {
        ...
        .onChange(of: client.currentWindow?.windowId) { newValue in
            // 1 cửa sổ GUI mới mở (rương, bàn chế tạo...) -> tự chuyển sang màn
            // Inventory để hiện nó, giống hành vi ChatCraft.
            if newValue != nil { screen = .inventory }
        }
        .onAppear {
