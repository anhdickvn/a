...
private struct ChatTranscriptTextView: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: UITextView, context: Context) {
        ...
        DispatchQueue.main.async {
            ...
            // First population / explicit send: go to the latest message.
            ...
            // New server messages while the user is reading older chat: preserve the
            // exact viewport. Never scroll to the bottom and never discard old entries here.
            if !previousAtBottom {
            ...
            // User was already at the bottom: follow new messages.
            let length = uiView.textStorage.length
            if length > 0 {
                uiView.scrollRangeToVisible(NSRange(location: length - 1, length: 1))
            }
