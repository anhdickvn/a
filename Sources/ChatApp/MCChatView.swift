...
struct MCChatView: View {
    ...
    private var chatView: some View {
        VStack(spacing: 0) {
            ...
            // thay vì chỉ bôi đen được từng dòng riêng lẻ như trước.
            ...
            if !visibleLog.isEmpty {
                HStack(spacing: 24) {
                    ...
                    Button {
                        UIPasteboard.general.string = visibleLog.map(\.text).joined(separator: "\n")
                    } label: {
                        Image(systemName: "doc.on.doc")
                            .font(.title2.weight(.semibold))
                            .foregroundStyle(.blue)
                    }
                    .accessibilityLabel("Copy toàn bộ chat")
                    Spacer()
                }
                .padding(.vertical, 10)
                .background(Color.black)
...
private struct ChatTranscriptTextView: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: UITextView, context: Context) {
        ...
        if previousAtBottom || shouldForceScroll {
            DispatchQueue.main.async {
                let length = uiView.textStorage.length
                if length > 0 {
                    let range = NSMakeRange(length - 1, 1)
    ...
    private func makeTranscriptAttributedString() -> NSAttributedString {
        ...
        for (index, entry) in entries.enumerated() {
            let message = (entry.segments?.isEmpty == false) ? attributedMessage(entry.segments!) : NSAttributedString(string: entry.text, attributes: [
                ...
                .foregroundColor: UIColor(entry.kind == .error ? .systemRed : .white)
            ])
