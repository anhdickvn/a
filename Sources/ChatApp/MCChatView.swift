...
struct MCChatView: View {
    ...
    private var chatView: some View {
        VStack(spacing: 0) {
            ...
            if showTabSuggestions && !client.tabCompletions.isEmpty && input.hasPrefix("/") {
                VStack(spacing: 0) {
    ...
    private func completeTab() {
        guard client.state == .connected, input.hasPrefix("/") else { return }
