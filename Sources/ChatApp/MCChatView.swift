...
struct MCChatView: View {
    ...
    private var chatView: some View {
        VStack(spacing: 0) {
            ...
            // ChatCraft-style: tin mới nằm ở ĐÁY. Quan trọng: nếu người dùng đang
            // kéo lên đọc lịch sử thì tuyệt đối KHÔNG tự kéo xuống khi có tin mới.
            GeometryReader { viewport in
                ScrollViewReader { proxy in
                                ...
                                .background(GeometryReader { geo in
                                    Color.clear.preference(
                                        ...
                                        value: ChatScrollMetrics(
                                            contentBottom: geo.frame(in: .named("chatScroll")).maxY,
                    ...
                    .onPreferenceChange(ChatScrollMetricsKey.self) { metrics in
                        // 40pt tolerance để một cú vuốt nhỏ gần đáy vẫn được coi là
                        // đang ở cuối. Nếu content bottom trôi xa hơn viewport, user
                        // đang đọc lịch sử -> giữ nguyên vị trí khi tin mới tới.
                        chatIsAtBottom = metrics.contentBottom <= metrics.viewportHeight + 40
                    }
                    .onChange(of: visibleLog.count) { newCount in
                        ...
                        lastChatVisibleCount = newCount
                        guard chatIsAtBottom else { return }
...
private struct ChatScrollMetricsKey: PreferenceKey {
    static var defaultValue = ChatScrollMetrics(contentBottom: 0, viewportHeight: 0)
