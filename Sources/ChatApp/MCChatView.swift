...
struct MCChatView: View {
    ...
    @AppStorage("mc_show_protocol_debug") private var showProtocolDebug = false
...
private struct ChatTranscriptTextView: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: UITextView, context: Context) {
        ...
        let previousLength = uiView.textStorage.length

        // Replacing attributedText can reset UITextView's scroll position. Preserve the
        // user's exact position whenever they are reading older chat; only jump to the
        // bottom when they were already at the bottom or explicitly sent a message/command.
        uiView.attributedText = makeTranscriptAttributedString()
        context.coordinator.isAtBottom = previousAtBottom
        ...
        let contentChanged = previousLength != uiView.textStorage.length
        if wasEmpty || previousAtBottom || shouldForceScroll {
            DispatchQueue.main.async {
                uiView.layoutIfNeeded()
                uiView.layoutManager.ensureLayout(for: uiView.textContainer)
                let length = uiView.textStorage.length
                if length > 0 {
                    uiView.scrollRangeToVisible(NSRange(location: length - 1, length: 1), animated: false)
                }
    ...
    private func attributedMessage(_ segments: [MCChatSegment]) -> NSAttributedString {
        ...
        for segment in segments {
            ...
            if let link = segment.linkURL.flatMap(URL.init(string:)) { attrs[.link] = link }
