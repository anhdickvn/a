...
private struct ChatTranscriptTextView: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: UITextView, context: Context) {
        ...
        // Chỉ tự cuộn khi trước đó người dùng đang ở cuối, hoặc khi người dùng vừa gửi
        // một tin/lệnh (force token). Vuốt lên giữa chừng thì tuyệt đối giữ nguyên vị trí.
        ...
        for (index, entry) in entries.enumerated() {
            ...
            if index < entries.count - 1 {
                result.append(NSAttributedString(string: "\n────────────\n", attributes: [
                    ...
                    .foregroundColor: UIColor.white.withAlphaComponent(0.16)
                ]))
                result.append(NSAttributedString(string: "\n", attributes: [
                    .font: UIFont.systemFont(ofSize: 18)
                ]))
