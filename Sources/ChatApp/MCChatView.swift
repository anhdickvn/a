...
private struct ChatTranscriptTextView: UIViewRepresentable {
    ...
    private func makeTranscriptAttributedString() -> NSAttributedString {
        ...
        result.enumerateAttribute(.link, in: NSRange(location: 0, length: result.length)) { value, range, _ in
            if value != nil { result.addAttribute(.underlineStyle, value: NSUnderlineStyle.single.rawValue, range: range) }
        }
        return result
    }
    ...
    private func attributedMessage(_ segments: [MCChatSegment]) -> NSAttributedString {
        ...
        for segment in segments {
            ...
            if let link = segment.linkURL.flatMap(URL.init(string:)) {
                attrs[.link] = link
                // Server-provided open_url links (e.g. Đăng nhập / Đăng ký / Đổi mật khẩu)
                // keep their original Minecraft color and are always visibly underlined.
