...
struct MCChatView: View {
    ...
    var body: some View {
        ...
        .confirmationDialog(
            windowActionItem.map { "Item: \($0.plainName)" } ?? "Item actions",
            isPresented: $showWindowItemActions,
            titleVisibility: .visible
        ) {
            if let item = windowActionItem {
                ...
                Button("Drop all", role: .destructive) { client.dropAllFromWindowSlot(item.slot) }
            }
            Button("Cancel", role: .cancel) {}
        }
        .confirmationDialog(
            "Item actions",
            isPresented: $showInventoryItemActions,
            titleVisibility: .visible
        ) {
            if let slot = inventoryActionSlot {
                Button("Display lore") {
                ...
                Button("Drop all", role: .destructive) { client.dropAllFromInventorySlot(slot) }
            }
            Button("Cancel", role: .cancel) {}
        }
        ...
        .confirmationDialog(
            ...
            Button("Stay connected") {
                // Stay chỉ đóng hộp thoại. Không reconnect, không disconnect, không đổi màn.
                showStayConnectedPrompt = false
                inputFocused = false
            }
            Button("Disconnect", role: .destructive) {
                // Ưu tiên phản hồi UI ngay: đóng dialog + bắt đầu pop trước, sau đó mới
                // dọn socket/log ở lượt run-loop kế tiếp. Tránh cảm giác nút Disconnect
                // bị treo vì NWConnection.cancel() + hàng loạt @Published update chạy
                // cùng trong transaction của confirmationDialog.
                showStayConnectedPrompt = false
                inputFocused = false
                input = ""
                historyIndex = nil
            ...
            Text("Stay sẽ đóng hộp thoại và giữ nguyên phiên chơi. Disconnect sẽ ngắt server, xóa chat của phiên hiện tại và quay về Accounts / Servers; Logs vẫn được giữ.")
        }
        .onAppear {
    ...
    private var chatView: some View {
        VStack(spacing: 0) {
            ...
            // ChatCraft-style: chat is anchored at the bottom and grows upward.
            // We invert the scroll view and the rows so the newest message is always
            // visually at the bottom; swiping upward reveals older messages.
            GeometryReader { viewport in
                ScrollViewReader { proxy in
                    ScrollView(.vertical, showsIndicators: true) {
                        LazyVStack(alignment: .leading, spacing: 0) {
                            ForEach(visibleLog.reversed()) { entry in
                                ...
                                if entry.id != visibleLog.first?.id {
                    ...
                    .onChange(of: visibleLog.count) { newCount in
                        ...
                        lastChatVisibleCount = newCount
                        // Only follow new messages when the user was already at the
                        // bottom. If they are reading older messages, do nothing.
                    ...
                    // The newest row is the visual bottom after inversion. If it is
                    // close to the lower edge of the chat viewport, follow live messages;
                    // otherwise the user is reading history and must not be moved.
                    .onPreferenceChange(ChatBottomMarkerKey.self) { markerY in
                        let screenHeight = UIScreen.main.bounds.height
                        chatIsAtBottom = abs(markerY - screenHeight) < 220
                    }
                }
            }
    ...
    private func serverWindowRow(_ item: MCOpenWindowItem) -> some View {
        HStack(spacing: 8) {
            ...
            Spacer(minLength: 4)
            Button {
                windowActionItem = item
                showWindowItemActions = true
            } label: {
                ...
                ForEach(Array(slots), id: \.self) { slot in
                    if let item = client.playerInventory[slot] {
                        HStack(spacing: 10) {
                            ...
                            Spacer()
                            Button {
                                inventoryActionSlot = slot
                                showInventoryItemActions = true
                            } label: {
                                Image(systemName: "ellipsis")
                                    .font(.headline.weight(.bold))
                                    .foregroundStyle(.blue)
