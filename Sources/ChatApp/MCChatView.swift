...
struct MCChatView: View {
    ...
    @State private var commandScrollRequest = 0
        ...
        VStack(spacing: 0) {
            ...
            GeometryReader { viewport in
                ...
                .onChange(of: visibleLog.count) { newCount in
                    ...
                    lastChatVisibleCount = newCount
                    // Tin mới chỉ tự cuộn khi người dùng đang ở sát đáy.
                    // Nếu đang kéo lên đọc lịch sử, tuyệt đối giữ nguyên viewport.
                    guard chatIsAtBottom, let newest = visibleLog.last else { return }
                    DispatchQueue.main.async {
                        proxy.scrollTo(newest.id, anchor: .bottom)
                    }
                }
                .onChange(of: commandScrollRequest) { _ in
                    // Người dùng vừa gửi command: luôn đưa về tin mới nhất, kể cả
                    // trước đó đang đứng ở giữa lịch sử chat.
                    guard let newest = visibleLog.last else { return }
                    DispatchQueue.main.async {
                        proxy.scrollTo(newest.id, anchor: .bottom)
                        chatIsAtBottom = true
                    }
    ...
    private func sendMessage() {
        ...
        input = ""
        // Command là hành động mới của người dùng: luôn đưa chat về cuối để thấy
        // phản hồi của server. Quy tắc này không ảnh hưởng tin nhắn server tự gửi.
        if isCommand {
            commandScrollRequest &+= 1
        }
    }
