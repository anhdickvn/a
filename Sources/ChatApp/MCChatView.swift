    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarLeading) {
                Button {
                    // Nếu bàn phím đang mở (đang gõ chat) mà bấm "<", cái chạm đầu tiên trên
                    // iOS thường chỉ để resign first responder của MCTabTextField (đóng bàn
                    // phím) chứ KHÔNG chuyển tới action của nút — phải bấm lần 2 mới thấy hộp
                    // thoại Stay connected/Disconnect hiện ra. Ở đây tự resign trước, rồi mới
                    // xử lý phần còn lại của action ở tick kế tiếp để chắc chắn SwiftUI nhận
                    // đúng lần chạm đầu tiên.
                    inputFocused = false
        ...
        .onAppear {
            ...
            guard !client.isConnectedTo(host: profile.host, port: profile.port, username: account.username) else {
                // Đã "Stay connected" từ trước (không disconnect) -> không gọi connect() lại,
                // nhưng vẫn phải đồng bộ toggle Autologin mới nhất từ profile, kẻo tắt Autologin
                // trong Edit server mà app vẫn tự chạy automation cũ ngầm bên dưới.
                client.syncAutoLogin(profile.autoLogin)
                return
            }
            ...
            GeometryReader { viewport in
                ...
                .onAppear {
                    ...
                    .onPreferenceChange(ChatBottomMarkerKey.self) { markerY in
                        // The marker is measured in the ScrollView's named viewport
                        // coordinate space. Near the viewport bottom = following live chat.
                        chatIsAtBottom = markerY <= viewport.size.height + 8 && markerY >= viewport.size.height - 260
                    }
                }
            }
