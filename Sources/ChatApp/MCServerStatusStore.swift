...
struct MCServerStatus: Equatable, Sendable {
    ...
    var motd: String = ""
    var motdSegments: [MCChatSegment] = []
    var onlinePlayers: Int = 0
    var maxPlayers: Int = 0
    var pingMs: Int = 0
    var versionName: String = ""
    /// Server favicon trả về từ Status Response, dạng data:image/png;base64,...
...
private final class MCStatusPingSession {
    ...
    func run() async -> MCServerStatus {
        await withCheckedContinuation { continuation in
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 5) { [weak self] in
                self?.finish(MCServerStatus(phase: .offline("Hết thời gian chờ")))
            }
        }
    }
    ...
    private func handleState(_ state: NWConnection.State) {
        ...
        case .failed, .cancelled:
            finish(MCServerStatus(phase: .offline("Không kết nối được")))
        default:
            break
        }
    }
    ...
    private func receiveNext() {
        connection.receive(minimumIncompleteLength: 1, maximumLength: 65536) { [weak self] data, _, isComplete, error in
            Task { @MainActor in
                ...
                if error != nil {
                    self.finish(MCServerStatus(phase: .offline("Không phản hồi")))
                    return
                }
                if isComplete {
                    self.finish(self.awaitingPong ? self.baseStatus : MCServerStatus(phase: .offline("Đóng kết nối")))
                    return
                }
    ...
    private func handleStatusResponse(_ body: Data) {
        guard let (json, _) = body.readMCString(from: 0),
            ...
            finish(MCServerStatus(phase: .offline("Phản hồi không hợp lệ")))
            return
        }
        let players = obj["players"] as? [String: Any]
        var faviconData: Data?
        if let favicon = obj["favicon"] as? String, let comma = favicon.firstIndex(of: ",") {
            let base64 = String(favicon[favicon.index(after: comma)...])
            faviconData = Data(base64Encoded: base64)
        }
        let versionInfo = obj["version"] as? [String: Any]
        let motd: String
        let motdSegments: [MCChatSegment]
        if let descString = obj["description"] as? String {
            motdSegments = chatSegments(from: descString)
            motd = motdSegments.map(\.text).joined()
            ...
            motdSegments = chatSegments(from: descJSONString)
            motd = motdSegments.map(\.text).joined()
        } else {
            motd = ""
            motdSegments = []
