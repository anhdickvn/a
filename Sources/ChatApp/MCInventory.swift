...
struct MCItemSlot: Identifiable, Equatable {
    ...
    var count: Int
    /// Bytes gốc của Slot từ server, dùng khi gửi Click Window để server nhận đúng NBT/count/damage.
    var rawSlotBytes: Data = Data()
...
private func enchantmentLines(from node: MCNBT?) -> [[MCChatSegment]]? {
    guard case .list(let values) = node else { return nil }
    var lines: [[MCChatSegment]] = []
    for value in values {
        guard case .compound(let dict) = value,
              case .string(let rawID)? = dict["id"] else { continue }
        let lvl: Int = {
            if case .short(let n)? = dict["lvl"] { return Int(n) }
            if case .int(let n)? = dict["lvl"] { return Int(n) }
            if case .byte(let n)? = dict["lvl"] { return Int(n) }
            return 1
        }()
        ...
        let name = id.replacingOccurrences(of: "_", with: " ")
            ...
            .joined(separator: " ")
        let roman: String
        switch lvl { case 1: roman = "I"; case 2: roman = "II"; case 3: roman = "III"; case 4: roman = "IV"; case 5: roman = "V"; default: roman = String(lvl) }
        lines.append([MCChatSegment(text: "\(name) \(roman)", colorHex: "#5555FF")])
    }
    return lines
}
...
enum MCSlotParser {
    ...
    static func parse(_ data: Data, from offset: Int, hotbarIndex: Int) -> (MCItemSlot?, Int)? {
        ...
        if firstByte == 0 {
            ...
            if let enchantments = enchantmentLines(from: root.get("Enchantments")) {
                loreSegments.append(contentsOf: enchantments)
            }
        }

        let raw = data.subdata(in: offset..<finalOffset)
        let item = MCItemSlot(hotbarIndex: hotbarIndex, itemId: blockId, damage: damage, count: Int(countByte),
                               rawSlotBytes: raw, nameSegments: displayNameSegments, loreSegments: loreSegments)
