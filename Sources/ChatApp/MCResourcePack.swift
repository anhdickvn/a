...
final class MCResourcePackStore: ObservableObject {
    ...
    func importPack(from zipURL: URL) async throws {
        ...
        // Same reasoning as ensureVanillaAssets: don't unzip on the MainActor.
    ...
    func installDownloadedPack(zipURL: URL, name: String) async throws {
        ...
        // on MainActor: server resource packs are a common source of the "app treo/đơ"
        // symptom when a GUI opens immediately after login.
        try await Task.detached(priority: .utility) {
            try FileManager.default.createDirectory(at: folder, withIntermediateDirectories: true)
            try FileManager.default.unzipItem(at: zipURL, to: folder)
        }.value
    ...
    func ensureVanillaAssets() async {
        ...
        do {
            ...
            // Unzipping the full ~7MB vanilla client.jar (tens of thousands of entries)
            // and then walking every extracted file to build the texture index are both
            // slow, synchronous filesystem operations. Running them directly here would
            // block the MainActor (this whole function runs on it) and freeze the UI for
            // several seconds right after opening a server — exactly the "mới vô bị
            // treo/đơ" symptom. Do both off the main actor and only touch @Published /
            // cache state back on the main actor once the result is ready.
            let index: [String: URL] = try await Task.detached(priority: .utility) {
                ...
                return MCResourcePackStore.buildIndexStatic(extracted)
            }.value
            applyVanillaIndex(index)
        } catch {
    ...
    private func buildIndex(_ folder: URL) -> [String: URL] {
        MCResourcePackStore.buildIndexStatic(folder)
    }

    /// Same scan as `buildIndex`, but `nonisolated` so it can run inside a `Task.detached`
    /// off the MainActor — this walk over thousands of files is the other half of what
    /// used to freeze the UI on first connect (see `ensureVanillaAssets`).
    nonisolated static func buildIndexStatic(_ folder: URL) -> [String: URL] {
    ...
    private func loadVanillaIndex(_ folder: URL) {
        applyVanillaIndex(buildIndex(folder))
    }

    private func applyVanillaIndex(_ index: [String: URL]) {
        vanillaFiles = index
