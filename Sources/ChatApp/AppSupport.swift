    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("Chẩn đoán") {
                    Toggle("Hiện Debug protocol", isOn: $showProtocolDebug)
                    Text("Tắt để ẩn các dòng T+..., RX/TX packet, diagnostic và thông tin kỹ thuật khỏi khung chat. Debug vẫn được ghi trong phiên để bật lại khi cần kiểm tra lỗi.")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if client.movementDebugLines.isEmpty {
                ContentUnavailableView("Chưa có debug", systemImage: "gamecontroller", description: Text("Bật “Ghi Debug WASD / Jump”, vào server và bấm W/A/S/D hoặc JUMP."))
            } else {
                ScrollViewReader { proxy in
                    ...
                    .onChange(of: client.movementDebugLines.count) { _, _ in
