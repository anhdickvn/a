...
final class MCClient: ObservableObject {
    ...
    /// Heartbeat movement theo nhịp vanilla: khoảng 20 ticks (1 giây) khi đứng yên.
    /// Quan trọng: chỉ gửi sau khi đã nhận vị trí đầu tiên từ server; không bao giờ
    /// gửi vị trí giả X/Y/Z = 0 trong giai đoạn Login -> Play.
    private func startIdleAliveTicker() {
        ...
        idleAliveTask = Task { @MainActor [weak self] in
            while !Task.isCancelled {
                ...
                // Khi đứng yên, vanilla protocol 340 gửi packet Player (0x0C),
                // chỉ chứa On Ground. Không gửi XYZ bằng 0x0C và cũng không cần
                // gửi Player Position (0x0D) lặp lại nếu vị trí không đổi.
