...
final class MCClient: ObservableObject {
    ...
    /// Debug riêng cho WASD / Jump. Không đưa vào chat khi debug thường đang tắt.
    @Published private(set) var movementDebugLines: [String] = []
    private var movementDebugBuffer: [String] = []
    ...
    // Movement state is deliberately NOT @Published. Publishing every 50ms makes SwiftUI
    // rebuild the WASD buttons while the finger is still down, which ends the UIKit touch
    // and is exactly why the old build only moved for a few packets.
    var playerX: Double = 0
    var playerY: Double = 0
    var playerZ: Double = 0
    var playerYaw: Float = 0
    var playerPitch: Float = 0
    @Published private(set) var displayedPlayerX: Double = 0
    @Published private(set) var displayedPlayerY: Double = 0
    @Published private(set) var displayedPlayerZ: Double = 0
    ...
    private var movementSequence: Int = 0
    private var lastMovementDebugAt: TimeInterval = 0
    private var microMovementGeneration: Int = 0
    private var movementSuppressedUntil: TimeInterval = 0
    private var movementLastSentX: Double = 0
    private var movementLastSentZ: Double = 0
    // Last server-authoritative position, used by movement diagnostics to distinguish
    // a client-side movement from a server correction/rollback.
    private var movementLastAuthoritativeX: Double = 0
    private var movementLastAuthoritativeZ: Double = 0
    private var movementProbeTask: Task<Void, Never>?
    // Movement input state used by the 20 TPS controller. These must live on the
    // client itself so SwiftUI re-renders do not recreate/lose the touch state.
    private var activeMovementInputs: [String: (forward: Double, strafe: Double)] = [:]
    private var movementControlTask: Task<Void, Never>?
    private var movementBurstTasks: [String: Task<Void, Never>] = [:]
    ...
    private func setupBackgroundObservers() {
        ...
        backgroundObservers.append(nc.addObserver(forName: UIApplication.willResignActiveNotification, object: nil, queue: .main) { [weak self] _ in
            Task { @MainActor [weak self] in
                guard let self else { return }
                guard self.shouldStayConnected else { return }
                self.isAppInBackground = true
                self.enteredBackgroundAt = self.enteredBackgroundAt ?? Date()
                // Start the audio background session BEFORE iOS suspends the app (e.g.
                // immediately when Safari/another app is opened from a chat URL).
                // This gives the TCP receive loop the best chance to remain alive.
                BackgroundKeepAlive.shared.start()
                self.stopWatchdog()
                self.stopIdleAliveTicker()
                self.appendLog(.info, "App sắp xuống nền: bật giữ nền trước khi hệ thống suspend phiên TCP.")
            }
        })
        backgroundObservers.append(nc.addObserver(forName: UIApplication.didEnterBackgroundNotification, object: nil, queue: .main) { [weak self] _ in
            Task { @MainActor [weak self] in
                ...
                // Khi chuyển app/Home, cố gắng giữ phiên lâu nhất có thể.
                // Không hủy socket/reconnect ngay; bật background audio session đã có sẵn
                // trong project để iOS có cơ hội tiếp tục tiến trình TCP. iOS vẫn có thể
                // đình chỉ/kết thúc app theo chính sách hệ thống, nên đây không phải bảo đảm vô hạn.
                self.stopWatchdog()
                self.stopIdleAliveTicker()
                if self.shouldStayConnected {
                    BackgroundKeepAlive.shared.start()
                    self.appendLog(.info, "App vào nền: giữ phiên TCP, không chủ động disconnect.")
                }
            }
        })
        backgroundObservers.append(nc.addObserver(forName: UIApplication.willEnterForegroundNotification, object: nil, queue: .main) { [weak self] _ in
            Task { @MainActor [weak self] in
                ...
                guard self.shouldStayConnected else { return }
                // Nếu socket chết trong lúc app ở nền, không hiện lỗi ngay và không kick người dùng khỏi GUI.
                // Chỉ reconnect âm thầm khi quay lại foreground.
                let silentFor = Date().timeIntervalSince(self.lastPacketReceivedAt)
                if self.pendingBackgroundNetworkFailure != nil || self.connection == nil ||
                    (self.state == .connected && (backgroundDuration >= 60 && silentFor >= 60)) {
                    self.pendingBackgroundNetworkFailure = nil
                    self.scheduleReconnect(reason: "socket stale sau khi app chạy nền >60s")
                } else if self.state == .connected {
                    self.startWatchdog()
                    self.startIdleAliveTicker()
                } else if self.state != .connecting && self.state != .loggingIn {
                    self.appendLog(.info, "Quay lại app: giữ phiên hiện tại, không reconnect ngay.")
                    self.startWatchdog()
                    self.startIdleAliveTicker()
                }
            }
        })
        backgroundObservers.append(nc.addObserver(forName: UIApplication.didBecomeActiveNotification, object: nil, queue: .main) { [weak self] _ in
            Task { @MainActor [weak self] in
                guard let self else { return }
    ...
    /// Xóa toàn bộ chat/system/error của phiên hiện tại nhưng giữ lại debug trace.
    /// Debug có thể bật lại trong Settings để điều tra nguyên nhân disconnect.
    func clearCurrentChatMessagesPreservingDebug() {
        log.removeAll { entry in
            entry.kind != .debug
        }
        sentMessageHistory.removeAll()
        tabCompletions.removeAll()
        ...
        conn.viabilityUpdateHandler = { [weak self] viable in
            ...
            Task { @MainActor [weak self] in
                ...
                if self.isAppInBackground {
                    // iOS may report the TCP path as temporarily non-viable while the
                    // app is backgrounded. Do NOT tear down/reconnect here; ChatCraft
                    // keeps the session object alive and lets iOS resume the socket.
                    self.pendingBackgroundNetworkFailure = "NWConnection non-viable while backgrounded"
                    return
                }
    ...
    private func handleConnectionState(_ nwState: NWConnection.State) {
        ...
        case .failed(let error):
            if isAppInBackground {
                pendingBackgroundNetworkFailure = error.localizedDescription
                // Keep the user-facing session alive while iOS has the app suspended.
                // Foreground handler decides whether a silent reconnect is needed.
                return
            }
    ...
    // Protocol diagnostics intentionally disabled in this build.
    // Keep the helpers as no-ops so the existing connection/protocol logic is unchanged.
    private func resetProtocolTrace() {}
    private func tracePacket(direction: String, id: Int32, payloadBytes: Int, note: String = "") {}
    private func protocolPacketName(_ id: Int32, direction: String) -> String { "" }
    private func dumpProtocolTrace(trigger: String) {}
    ...
    private func send(packetId: Int32, payload: [UInt8]) {
        ...
        let isMovementPacket = (packetId == 0x0D || packetId == 0x0E || packetId == 0x0C)
        var movementSeq: Int? = nil
        if isMovementPacket && movementDebugEnabled {
            movementSequence += 1
            movementSeq = movementSequence
            movementDebug("WIRE TX \(String(format: "0x%02X", packetId)) seq=\(movementSequence) name=\(movementPacketName(packetId)) payloadBytes=\(payload.count) connection=\(connection == nil ? "nil" : "present")", force: true)
            if payload.count <= 64 {
                movementDebug("WIRE PAYLOAD HEX=\(payload.map { String(format: "%02X", $0) }.joined(separator: " "))", force: true)
            }
        }
        connection?.send(content: Data(full), completion: .contentProcessed { [weak self] error in
            guard let self else { return }
            guard isMovementPacket && self.movementDebugEnabled else {
                if let error {
                    Task { @MainActor [weak self] in
                        guard let self else { return }
                        if self.isAppInBackground {
                        ...
                        if case .posix(let posix) = error, posix.rawValue == 89, !self.shouldStayConnected { return }
            ...
            Task { @MainActor [weak self] in
                ...
                if let movementSeq {
                    if let error {
                        self.movementDebug("WIRE RESULT seq=\(movementSeq): ERROR \(error.localizedDescription)", force: true)
                    } else {
                        self.movementDebug("WIRE RESULT seq=\(movementSeq): SENT OK", force: true)
                    }
                }
                ...
                self.appendLog(.error, self.chatCraftNetworkErrorMessage(error))
                if self.shouldStayConnected, self.state != .connecting, self.state != .loggingIn {
                    self.scheduleReconnect(reason: "send packet lỗi")
                }
            }
    ...
    private func chatCraftNetworkErrorMessage(_ error: NWError) -> String {
        if case .posix(let posix) = error {
            let detail: String
            switch posix.rawValue {
            case 89: detail = "Operation canceled by system/peer"
            case 53: detail = "Connection reset by peer"
            default: detail = String(describing: posix)
            }
            return "Lỗi kết nối: The operation couldn’t be completed. (Network.NWError error \(posix.rawValue) - \(detail))"
        }
        return "Lỗi kết nối: The operation couldn’t be completed. (\(error.localizedDescription))"
    }
    ...
    ///
    /// FIX (v25): trước đây ticker này chỉ chạy SAU KHI đã nhận vị trí đầu tiên từ
    /// server (`hasServerPosition`). Khoảng thời gian từ lúc Login Success cho tới khi
    /// server gửi Player Position And Look — đúng lúc luồng auto-login (/dn -> mở la
    /// bàn -> bấm Diamond Axe) đang chạy — hoàn toàn không có packet di chuyển nào được
    /// gửi đi (kể cả 0x0C cũng không, vì hàm gửi nó chưa từng được gọi ở đâu). Với
    /// plugin/proxy chống bot theo dõi nhịp tick di chuyển ngay từ khi vào Play, khoảng
    /// lặng vài giây này đúng là lúc bị đánh dấu bot và lệnh bị chặn.
    ///
    /// Giờ ticker chạy ngay sau Login Success: khi CHƯA có vị trí thật từ server thì
    /// gửi 0x0C (không kèm toạ độ nên không có rủi ro bị nghi "moved wrongly"/anti
    /// speed-hack); khi ĐÃ có vị trí thật thì chuyển sang gửi đủ 0x0D như cũ. Nhờ vậy
    /// không còn khoảng lặng nào giữa lúc vào Play và lúc tự động bấm menu.
    private func startIdleAliveTicker() {
        // Keep Alive packets are handled from the server. Do not synthesize
        // periodic player-position packets while idle; those packets can race
        // the server-authoritative position and create correction/disconnect loops.
        idleAliveTask?.cancel()
        idleAliveTask = nil
    }
    ...
    private func stopIdleAliveTicker() {
        ...
        // Current movement controller is allowed to resume after the correction window.
    }
    ...
    private func startHandshakeAndLogin() {
        movementDebug("LOGIN PROTOCOL=\(protocolVersion) versionHint=\(versionHint(for: protocolVersion))", force: true)
        state = .loggingIn
            ...
            Task { @MainActor in
                ...
                if let error {
                    ...
                    if self.isAppInBackground {
                        ...
                        self.stopWatchdog()
                        self.stopIdleAliveTicker()
                        // Giữ phiên logic và UI yên cho tới khi người dùng quay lại app.
                        // Đây là điểm quan trọng để không hiện disconnect ngay lúc Home/đổi app.
                        return
                    }
                    self.appendLog(.error, self.chatCraftNetworkErrorMessage(error))
                    ...
                    self.clearCurrentChatMessagesPreservingDebug()
                    self.state = .failed(error.localizedDescription)
                    return
                }
                if isComplete {
                    if self.state != .disconnected {
                        self.dumpProtocolTrace(trigger: "TCP EOF / server đóng socket (không có Disconnect packet trước EOF)")
                        if self.isAppInBackground {
                            self.pendingBackgroundNetworkFailure = "TCP EOF / server đóng socket"
                            self.connection = nil
                            self.stopWatchdog()
                            self.stopIdleAliveTicker()
                            return
                        }
                        self.appendLog(.error, "Lỗi kết nối: The operation couldn’t be completed. (TCP EOF - server đóng socket)")
                        self.shouldStayConnected = false
                        self.reconnectWorkItem?.cancel()
                        self.reconnectWorkItem = nil
    ...
    private func handleLoginPacket(id: Int32, body: Data) {
        ...
        case 0x02: // Login Success -> chuyển sang Play
            ...
            state = .connected
            lastPacketReceivedAt = Date()
            debugSessionStart = Date()
            resetProtocolTrace()
            dbg("Login Success — vào Play. Bắt đầu debug trace phiên này.")
            startWatchdog()
            startIdleAliveTicker()
            // Không cố giữ TCP bằng audio giả. iOS có thể suspend socket trong nền;
            // khi quay lại foreground, connection watcher sẽ reconnect âm thầm.
            BackgroundKeepAlive.shared.stop()
            attemptAutoLogin()
    ...
    private func handlePlayPacket(id: Int32, body: Data) {
        if movementDebugEnabled {
            movementDebug("RX PLAY packet=0x\(String(format: "%02X", id)) bytes=\(body.count)", force: true)
            if id == 0x2F || id == 0x23 || id == 0x35 || id == 0x1F || id == 0x1A {
                movementDebug("RX MOVEMENT-RELEVANT type=\(movementPacketName(id))", force: true)
            }
        }
        switch id {
        ...
        case 0x11: // Confirm Transaction (clientbound) — protocol 340
            handleConfirmTransaction(body)
        ...
        case 0x0F: // Chat Message (clientbound)
            ...
            if trimmed.isEmpty {
                // Có thể do server dùng "translate" key mà parser chưa xử lý tốt — log
                // raw JSON dạng debug thay vì âm thầm bỏ qua, để không mất manh mối.
                dbg("Nhận chat message rỗng sau khi parse. Raw JSON: \(json)")
                return
            }
            appendLog(position == 1 ? .system : .chat, text, segments: segments)
            if isBlockedCommandMessage(trimmed) {
                dumpDiagnosticSnapshot(trigger: "Chat message chứa dấu hiệu lệnh bị chặn: \"\(trimmed.prefix(120))\"")
            }
        ...
        case 0x1A: // Disconnect (play)
            ...
            dumpProtocolTrace(trigger: "Server gửi Disconnect (0x1A): \(readableReason.prefix(160))")
            if isBlockedCommandMessage(readableReason) {
                dumpDiagnosticSnapshot(trigger: "Bị kick với lý do chứa dấu hiệu lệnh bị chặn: \"\(readableReason.prefix(120))\"")
            }
            ...
            // Khi server đã gửi Disconnect (0x1A), TUYỆT ĐỐI không auto-reconnect.
            // Giữ nguyên màn chat + toàn bộ debug trace để người dùng đọc/copy nguyên nhân.
            // Chỉ một lần connect thủ công mới bắt đầu phiên mới.
            connection?.cancel()
            connection = nil
            compressionThreshold = -1
            compressionFailureCount = 0
            ...
            reconnectWorkItem?.cancel()
            reconnectWorkItem = nil
            // Giữ nguyên transcript khi server disconnect để người dùng vẫn đọc được
            // tin nhắn và lý do kick, giống hành vi ChatCraft.
            state = .disconnected
    ...
    /// Serverbound/clientbound Confirm Transaction của protocol 340 (1.12.x).
    /// Client phải echo lại Window ID + Action Number + Accepted khi server gửi
    /// Confirm Transaction (đặc biệt với Window 0/player inventory). Nếu không trả
    /// lời, một số server/proxy sẽ tiếp tục gửi các transaction chưa xác nhận và
    /// cuối cùng disconnect với lý do disconnect.timeout, ngay cả khi người chơi
    /// hoàn toàn đứng yên và không mở GUI.
    private func handleConfirmTransaction(_ body: Data) {
        guard body.count >= 4 else {
            dbg("RX Confirm Transaction không đủ dữ liệu: bytes=\(body.count)")
            return
        }

        guard let windowID = body.readU8(from: 0),
              let action = body.readI16BE(from: windowID.1),
              let accepted = body.readU8(from: action.1) else {
            dbg("Không parse được Confirm Transaction: bytes=\(body.count)")
            return
        }

        let actionNumber = action.0
        let acceptedByte = accepted.0

        // Vanilla 1.12.x only answers a rejected (accepted=false) transaction,
        // and its C0F Confirm Transaction ACK uses accepted=true.  Echoing the
        // server's false value is not what the Notchian client does and can cause
        // some proxies/servers to reset the socket immediately.
        guard acceptedByte == 0 else {
            dbg("RX Confirm Transaction: window=\(windowID.0) action=\(actionNumber) accepted=true → không cần ACK")
            return
        }

        var payload: [UInt8] = [windowID.0]
        let bits = UInt16(bitPattern: actionNumber)
        payload.append(UInt8((bits >> 8) & 0xFF))
        payload.append(UInt8(bits & 0xFF))
        payload.append(1) // Vanilla C0FPacketConfirmTransaction(..., true)

        let note = "confirm window=\(windowID.0) action=\(actionNumber) accepted=false → ACK 0x05 accepted=true"
        dbg("RX Confirm Transaction: \(note)")
        send(packetId: 0x05, payload: payload)
    }
    ...
    private func handleJoinGame(_ body: Data) {
        ...
        dbg("Nhận Join Game (dimension=\(currentDimension)). Gửi Client Settings + MC|Brand...")
    ...
    private func handlePlayerPositionAndLook(_ body: Data) {
        ...
        let incomingBeforeApplyX = playerX
        let incomingBeforeApplyZ = playerZ
        ...
        if !hasServerPosition {
            dbg("Nhận Player Position And Look ĐẦU TIÊN từ server (x=\(String(format: "%.1f", playerX)), y=\(String(format: "%.1f", playerY)), z=\(String(format: "%.1f", playerZ)), teleportId=\(teleport.0)). Từ giờ heartbeat chuyển sang gửi 0x0D đầy đủ toạ độ.")
        }
        hasServerPosition = true
        displayedPlayerX = playerX
        displayedPlayerY = playerY
        displayedPlayerZ = playerZ
        movementLastAuthoritativeX = playerX
        movementLastAuthoritativeZ = playerZ
        movementDebug(String(format: "SERVER CORRECTION: x=%.3f y=%.3f z=%.3f yaw=%.1f pitch=%.1f flags=0x%02X teleport=%d", playerX, playerY, playerZ, playerYaw, playerPitch, flagByte, teleport.0), force: true)
        movementDebug("SERVER RESPONSE: Player Position And Look accepted as authoritative/correction", force: true)

        // Teleport Confirm — bắt buộc khi server gửi teleport id.
        // Tạm dừng movement/jump trong lúc server đang correction để không đấu lại
        // với vị trí authoritative và tạo vòng correction -> disconnect.
        // Cancel only an in-flight jump/burst. Do not clear held WASD state: the
        // movement controller can resume from the server-authoritative position after
        // the 100 ms correction window.
        jumpTask?.cancel()
        jumpTask = nil
        // A server correction is authoritative. Cancel any locally simulated jump and
        // resume from the corrected server state as grounded; otherwise the old
        // `isOnGround=false` survives the correction and every subsequent WASD packet
        // is sent with onGround=false, which can make 1.12.2 anti-cheat/proxy kick us.
        isOnGround = true
        movementProbeTask?.cancel()
        movementProbeTask = nil
        // Current movement controller is allowed to resume after the correction window.
        ...
        movementSuppressedUntil = Date().timeIntervalSince1970 + 0.06
    ...
    private func movementPacketName(_ id: Int32) -> String {
        guard protocolVersion == 340 else { return "unknown(protocol=\(protocolVersion))" }
        switch id {
        case 0x0D: return "Player Position (serverbound, protocol 340)"
        case 0x0E: return "Player Position And Look (serverbound, protocol 340)"
        case 0x0F: return "Player Look (serverbound, protocol 340)"
        case 0x10: return "Player (serverbound, protocol 340)"
        case 0x00: return "Teleport Confirm (serverbound, protocol 340)"
        default: return "unknown"
        }
    }

    private func logMovementContext(_ input: String) {
        movementDebug("--- MOVEMENT TEST BEGIN ---", force: true)
        movementDebug("PROTOCOL version=\(protocolVersion) fallback=\(fallbackProtocolVersion) loginProtocolIsHardcoded=true", force: true)
        movementDebug("PROTOCOL NOTE: current source does not negotiate client protocol; it forces 340. If server status advertises another protocol, this is a primary suspect for movement corrections.", force: true)
        movementDebug("INPUT=\(input)", force: true)
        movementDebug(String(format: "POS before x=%.4f y=%.4f z=%.4f yaw=%.2f pitch=%.2f onGround=%@ hasServerPosition=%@", playerX, playerY, playerZ, playerYaw, playerPitch, isOnGround ? "true" : "false", hasServerPosition ? "true" : "false"), force: true)
        movementDebug("CANDIDATE movement IDs (340): 0x0C=Player(onGround), 0x0D=Player Position, 0x0E=Player Position+Look, 0x0F=Player Look, 0x10=Vehicle Move", force: true)
    }
    ...
    /// Movement controller giống client Minecraft thông thường:
    /// - chạm W/A/S/D => gửi ngay 1 bước nhỏ;
    /// - giữ phím => 20 ticks/giây;
    /// - nhả phím => dừng ngay;
    /// - tap rất ngắn vẫn có một burst hữu hạn để server/plugin ghi nhận movement.
    func movePlayer(dx: Double, dz: Double) {
        guard state == .connected else {
            movementDebug("MOVE BLOCKED: state=\(state)", force: true)
            return
        }
        guard hasServerPosition else {
            movementDebug("MOVE BLOCKED: hasServerPosition=false", force: true)
            return
        }
        let now = Date().timeIntervalSince1970
        guard now >= movementSuppressedUntil else {
            movementDebug("MOVE BLOCKED: correction cooldown", force: true)
            return
        }
        let distance = hypot(dx, dz)
        guard distance > 0.0001 else { return }

        // Conservative micro-step: one small, server-visible move per tick.
        // The server on this network is sensitive to large/rapid position jumps.
        let maxStep = 0.10
        ...
        movementLastSentX = playerX
        movementLastSentZ = playerZ

        // Pure translation uses Player Position (0x0D), matching the 1.12.2 vanilla
        // movement packet. Rotation is sent separately only when needed.
        sendPlayerPosition(onGround: isOnGround)
        movementDebug(String(format: "MOVE TX \(movementPacketName(0x0D)): (%.3f,%.3f)->(%.3f,%.3f) step=%.3f onGround=%@",
                             oldX, oldZ, playerX, playerZ, scale * distance,
                             isOnGround ? "true" : "false"), force: true)
    }

    /// WASD theo yaw hiện tại của server.
    func movePlayer(forward: Double, strafe: Double) {
        guard state == .connected else {
            movementDebug("INPUT \(movementName(forward: forward, strafe: strafe)): state=\(state)", force: true)
            return
        }
        guard hasServerPosition else {
            movementDebug("INPUT \(movementName(forward: forward, strafe: strafe)): WAIT server position", force: true)
            return
        }
        ...
        let dx = forwardX * (f / length) + rightX * (s / length)
        let dz = forwardZ * (f / length) + rightZ * (s / length)
        logMovementContext(movementName(forward: forward, strafe: strafe))
        movementDebug("INPUT \(movementName(forward: forward, strafe: strafe)): yaw=\(String(format: "%.1f", yaw)) pos=(\(String(format: "%.2f", playerX)),\(String(format: "%.2f", playerY)),\(String(format: "%.2f", playerZ)))", force: true)
    ...
    /// Gửi movement đúng kiểu client: touch-down tạo chuyển động ngay;
    /// giữ phím thì tick 20 TPS; tap ngắn vẫn chạy thêm tối đa 4 bước hữu hạn.
    func startMoving(forward: Double, strafe: Double) {
        ...
        // Move immediately on touch-down, like a real launcher/client.
        sendCombinedMovementTick()
        ...
        if movementControlTask == nil {
            movementControlTask = Task { @MainActor [weak self] in
                while !Task.isCancelled {
                    ...
                    if !self.activeMovementInputs.isEmpty {
    ...
    func stopMoving(forward: Double, strafe: Double) {
        ...
        if activeMovementInputs.isEmpty {
            ...
            // Tap support: a very short touch still produces a small finite burst.
            let generation = UUID()
            movementBurstTasks[key]?.cancel()
            movementBurstTasks[key] = Task { @MainActor [weak self] in
                guard let self else { return }
                for _ in 0..<2 {
                    if Task.isCancelled || self.state != .connected || !self.hasServerPosition { break }
                    self.movePlayer(forward: max(-1, min(1, forward)), strafe: max(-1, min(1, strafe)))
                    try? await Task.sleep(nanoseconds: 50_000_000)
                }
                self.movementBurstTasks[key] = nil
                self.flushMovementDebug()
                _ = generation
            }
        }
    }

    /// Jump gần giống client Minecraft 1.12.2: impulse ngay khi nhấn SPACE,
    /// sau đó cập nhật quỹ đạo ở 20 TPS trong thời gian ngắn và hạ xuống ground.
    func jumpPlayer() {
        guard state == .connected else {
        ...
        isOnGround = false
        movementDebug(String(format: "JUMP DOWN start=(%.3f,%.3f,%.3f)", startX, startY, startZ), force: true)

        let firstY = startY + 0.42
        playerY = firstY
        displayedPlayerY = firstY
        movementLastSentAt = Date().timeIntervalSince1970
        movementLastSentX = playerX
        movementLastSentZ = playerZ
        sendPlayerPosition(onGround: false)
        movementDebug(String(format: "JUMP TX y=%.3f onGround=false", firstY), force: true)
        ...
        let work = DispatchWorkItem { [weak self] in
            ...
            if elapsed >= 0.55 {
                self.playerY = startY
                self.displayedPlayerY = startY
                self.isOnGround = true
                self.sendPlayerPosition(onGround: true)
                self.movementDebug(String(format: "JUMP LAND y=%.3f", startY), force: true)
                self.jumpTask = nil
                self.flushMovementDebug()
                return
            }

            // Vanilla-like vertical arc: v0≈0.42 blocks/tick, gravity≈0.08.
            let tick = min(elapsed / 0.05, 11.0)
            let dy = 0.42 * tick - 0.5 * 0.08 * tick * tick
            self.playerY = startY + max(0, dy)
            ...
            self.movementDebug(String(format: "JUMP TX t=%.2f y=%.3f", elapsed, self.playerY), force: true)
    ...
    private func jumpPlayerTick(generation: Int, startY: Double, startTime: Date) {
        guard state == .connected, self.jumpGeneration == generation else { return }
        let elapsed = Date().timeIntervalSince(startTime)
        if elapsed >= 0.55 {
            playerY = startY
            displayedPlayerY = startY
            isOnGround = true
            sendPlayerPosition(onGround: true)
            movementDebug(String(format: "JUMP LAND y=%.3f", startY), force: true)
            jumpTask = nil
            flushMovementDebug()
            return
        }
        ...
        let tick = min(elapsed / 0.05, 11.0)
        let dy = 0.42 * tick - 0.5 * 0.08 * tick * tick
        playerY = startY + max(0, dy)
        displayedPlayerY = playerY
        movementLastSentAt = Date().timeIntervalSince1970
        sendPlayerPosition(onGround: false)
        movementDebug(String(format: "JUMP TX t=%.2f y=%.3f", elapsed, playerY), force: true)

        jumpTask = DispatchWorkItem { [weak self] in
            self?.jumpPlayerTick(generation: generation, startY: startY, startTime: startTime)
        }
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.05, execute: jumpTask!)
    }
    ...
    private func sendPlayerPositionAndLook(onGround: Bool) {
        var payload: [UInt8] = []
        payload += playerX.mcBigEndianBytes
        payload += playerY.mcBigEndianBytes
        payload += playerZ.mcBigEndianBytes
        ...
        payload.append(onGround ? 1 : 0)
        send(packetId: 0x0E, payload: payload)
    }
    ...
    private func handleOpenWindow(_ body: Data) {
        ...
        dbg("Server mở GUI: windowId=\(windowId), title=\"\(prettyChatJSON(title))\", slotCount=\(slotCount).")
    }
    ...
    private func sendLoginAndScheduleMenu(password: String, token: UUID, generation: UUID) {
        ...
        dbg("Đã gửi \"/dn <mật khẩu>\". Chờ hasServerPosition=true (tối thiểu 2s, trần 6s) rồi mở la bàn...")
        // FIX (v25): trước đây đợi đúng 2 giây theo đồng hồ rồi mở la bàn, bất kể server
        // đã gửi Player Position And Look hay chưa. Nếu server/proxy xử lý /dn chậm hơn
        // 2 giây (mạng yếu, server đang tải), la bàn/Diamond Axe bị bấm trong lúc player
        // còn chưa thực sự "vào" Play theo góc nhìn của plugin chống bot -> dính "This
        // command cannot be executed". Giờ đợi tối thiểu 2s VÀ có vị trí thật
        // (`hasServerPosition`), với trần tối đa 6s để không treo vô thời hạn nếu vì lý
        // do gì đó server không gửi vị trí (khi đó vẫn mở la bàn như cũ, không đổi hành vi).
        waitForServerPositionThenOpenMenu(token: token, generation: generation, minDelay: 2.0, deadline: Date().addingTimeInterval(6.0))
    }

    private func waitForServerPositionThenOpenMenu(token: UUID, generation: UUID, minDelay: TimeInterval, deadline: Date) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) { [weak self] in
            guard let self, self.connectAttemptToken == token, self.loginAutomationGeneration == generation, self.state == .connected else { return }
            let minDelayElapsed = minDelay <= 0
            if (self.hasServerPosition && minDelayElapsed) || Date() >= deadline {
                if Date() >= deadline && !self.hasServerPosition {
                    self.dbg("⏱️ Hết hạn chờ 6s mà VẪN chưa có hasServerPosition — mở la bàn theo trần fallback (đây có thể là nguyên nhân).")
                } else {
                    self.dbg("hasServerPosition=true và đã qua tối thiểu 2s — mở la bàn ngay.")
                }
                self.startCompassAutomation(token: token, generation: generation)
            } else {
                self.waitForServerPositionThenOpenMenu(token: token, generation: generation, minDelay: minDelay - 0.25, deadline: deadline)
            }
        }
    }
    ...
    private func startCompassAutomation(token: UUID, generation: UUID) {
        let action = { [weak self] in
            ...
            guard let item = self.hotbar[4] else {
                self.dbg("Hotbar 5 hiện đang trống — chờ 0.5s rồi thử lại.")
            ...
            guard item.itemId == 345 else {
                ...
                self.dbg("Huỷ automation: hotbar 5 = itemId \(item.itemId), không phải Compass (345).")
                return
            }
            ...
            self.dbg("Đã chuột phải la bàn (hotbar 5, itemId=345). Chờ server mở GUI...")
            self.retryAutoMenuSelection(token: token, generation: generation)
        }
        if resourcePackPending {
            dbg("Resource pack đang tải dở — hoãn mở la bàn tới khi tải xong.")
    ...
    private func retryAutoMenuSelection(token: UUID, generation: UUID) {
        ...
        if let window = currentWindow, let item = window.items[23] {
            ...
            if isDiamondAxe {
                ...
                dbg("Đã CLICK Diamond Axe ở slot 23 (windowId=\(window.windowId)). Theo dõi phản hồi tiếp theo của server.")
                return
            } else {
                dbg("Ô 23 hiện có item khác Diamond Axe: itemId=\(item.itemId), tên=\"\(item.plainName)\" (retry \(autoMenuRetryCount)/24).")
            }
        }
        guard autoMenuRetryCount < 24 else {
            autoMenuSelectionPending = false
            appendLog(.error, "Không tìm thấy Diamond Axe ở ô GUI 23 sau 6 giây.")
            dbg("HẾT thời gian chờ (6s) — không tìm thấy Diamond Axe ở slot 23. currentWindow=\(currentWindow.map { "id=\($0.windowId) slots=\($0.slotCount)" } ?? "nil").")
            return
        }
    ...
    private func handleWindowItems(_ body: Data) {
        ...
        for slot in 0..<Int(count) {
            ...
            idx = next
            if let item {
                window.items[slot] = MCOpenWindowItem(slot: slot, itemId: item.itemId, damage: item.damage, count: item.count,
    ...
    /// Nhận diện thông điệp kiểu "This command cannot be executed on this server" (và
    /// vài cách proxy/plugin Việt hoá nó) để tự động chụp snapshot debug ngay lúc xảy ra,
    /// thay vì phải đoán lại sau khi nó đã trôi qua.
    private func isBlockedCommandMessage(_ text: String) -> Bool {
        let lower = text.lowercased()
        return lower.contains("cannot be executed") ||
            lower.contains("cannot execute") ||
            lower.contains("unknown command") ||
            lower.contains("không thể thực hiện lệnh") ||
            lower.contains("lệnh này không thể") ||
            lower.contains("không có quyền")
    }

    private func appendLog(_ kind: MCLogEntry.Kind, _ text: String, segments: [MCChatSegment]? = nil) {
        // Info nội bộ (click GUI, reconnect, texture pack...) không đưa lên chat.
        // ChatCraft-style UI chỉ hiển thị chat/system thực sự từ server, lỗi cần biết,
        // và (mới) debug trace — để người dùng chụp màn hình gửi đi khi cần chẩn đoán.
        guard kind != .info else { return }
        log.append(MCLogEntry(kind: kind, text: text, segments: segments))

        // The 500-line limit applies to the user-visible conversation, not hidden
        // protocol/debug entries. Otherwise a busy debug session silently evicts
        // older real chat messages from the main chat view.
        let visibleIndices = log.indices.filter {
            log[$0].kind == .chat || log[$0].kind == .system
        }
        if visibleIndices.count > 500 {
            let removeCount = visibleIndices.count - 500
            let idsToRemove = Set(visibleIndices.prefix(removeCount).map { log[$0].id })
            log.removeAll { idsToRemove.contains($0.id) }
        }

        // Keep diagnostic entries bounded separately so they do not grow forever.
        let debugCount = log.reduce(into: 0) { count, entry in
            if entry.kind == .debug { count += 1 }
        }
        if debugCount > 2000 {
            var remaining = debugCount - 2000
            log.removeAll { entry in
                guard remaining > 0, entry.kind == .debug else { return false }
                remaining -= 1
                return true
            }
        }
    ...
    /// Mốc T+0 cho debug trace — đặt lại mỗi lần Login Success (mỗi lần vào Play mới).
    private var debugSessionStart: Date?

    // Generic protocol/automation debug is disabled to avoid continuous background logging.
    // WASD / Jump debugging uses movementDebug() below and remains fully active when enabled.
    private func dbg(_ text: String) {}

    private func dumpDiagnosticSnapshot(trigger: String) {}
}
    ...
    while i < chars.count {
        if chars[i] == "\u{00A7}", i + 1 < chars.count {
            ...
            if let hex = mcColorHex[code] {
                // Minecraft color codes reset font styling, but they do not remove the
                // semantic click/hover attached to the component. Preserve those so
                // labeled URLs such as "Đăng nhập", "Đăng ký", "Đổi mật khẩu" remain
                // underlined/clickable even when the server changes color mid-component.
                let preservedLink = style.linkURL
                let preservedHover = style.hoverItem
                style = MCChatStyle(colorHex: hex)
                style.linkURL = preservedLink
                style.hoverItem = preservedHover
            } else {
                switch code {
...
private func walkChatComponent(_ obj: Any, style: MCChatStyle) -> [MCChatSegment] {
    ...
    if let hover = parseHoverItem(from: dict["hoverEvent"], fallbackStyle: st) {
        st.hoverItem = hover
    }
    ...
    if let translate = dict["translate"] as? String {
        // FIX (v25): bản cũ khi có "with" args thì THAY THẾ hoàn toàn translate key
        // bằng args, làm mất ngữ cảnh (không có bảng lang key -> value nên translate
        // key chỉ là 1 chuỗi thô như "commands.generic.permission", nhưng vẫn còn hơn
        // là im lặng biến mất). Giờ giữ cả hai để không bao giờ mất nội dung tin nhắn
        // gốc từ server — quan trọng khi debug lỗi lệnh bị chặn.
        let withArr = (dict["with"] as? [Any])?.flatMap { walkChatComponent($0, style: st) } ?? []
        segments += splitLegacyCodes(translate, base: st)
        if !withArr.isEmpty {
            segments += splitLegacyCodes(" ", base: st)
            segments += withArr
        }
    }
    ...
    return inlineItemHoverSegments(segments)
}


private func inlineItemHoverSegments(_ segments: [MCChatSegment]) -> [MCChatSegment] {
    let pattern = #"(?:tile\.|item\.)?([A-Za-z0-9_]+)(?:\.name)?\s+(\d+)\s*[xX]"#
    guard let re = try? NSRegularExpression(pattern: pattern) else { return segments }
    var result: [MCChatSegment] = []
    for segment in segments {
        let ns = segment.text as NSString
        let matches = re.matches(in: segment.text, range: NSRange(location: 0, length: ns.length))
        if matches.isEmpty || segment.hoverItem != nil {
        ...
        for match in matches {
            if match.range.location > cursor {
                ...
                result.append(MCChatSegment(text: prefix, colorHex: segment.colorHex, bold: segment.bold,
                                            italic: segment.italic, underline: segment.underline,
                                            strikethrough: segment.strikethrough, linkURL: segment.linkURL,
                                            hoverItem: segment.hoverItem))
            }
            let token = ns.substring(with: match.range)
            let rawName = ns.substring(with: match.range(at: 1))
            let count = Int(ns.substring(with: match.range(at: 2))) ?? 1
            let normalized = rawName.replacingOccurrences(of: "tile.", with: "").replacingOccurrences(of: "item.", with: "")
            let legacyKey = normalized.replacingOccurrences(of: "([a-z0-9])([A-Z])", with: "$1_$2", options: .regularExpression).lowercased()
            let hover = MCChatHoverItem(resourceName: "minecraft:\(normalized)",
                                        displayName: friendlyItemName(normalized), count: max(1, count),
                                        legacyItemId: legacyItemID(for: legacyKey), damage: 0,
                                        lore: [], enchantments: [], rawValue: token)
            result.append(MCChatSegment(text: token, colorHex: segment.colorHex, bold: segment.bold,
                                        italic: segment.italic, underline: segment.underline,
                                        strikethrough: segment.strikethrough, linkURL: segment.linkURL,
                                        hoverItem: hover))
        ...
        if cursor < ns.length {
            let suffix = ns.substring(with: NSRange(location: cursor, length: ns.length - cursor))
            result.append(MCChatSegment(text: suffix, colorHex: segment.colorHex, bold: segment.bold,
                                        italic: segment.italic, underline: segment.underline,
                                        strikethrough: segment.strikethrough, linkURL: segment.linkURL,
                                        hoverItem: segment.hoverItem))
        }
...
private func parseHoverItem(from value: Any?, fallbackStyle: MCChatStyle) -> MCChatHoverItem? {
    guard let dict = value as? [String: Any],
          let action = dict["action"] as? String,
          action == "show_item" else { return nil }

    // 1.13+ style may put the object in `contents`; 1.12.x commonly uses `value` as a JSON/NBT string.
    if let contents = dict["contents"] as? [String: Any] {
        return parseHoverItemObject(contents)
    }
    if let rawValue = dict["value"] as? String {
        if let data = rawValue.data(using: .utf8),
           let obj = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
           let item = parseHoverItemObject(obj) {
        ...
        return parseLegacyHoverItemString(rawValue)
    }
    return nil
}

private func parseChatAnyForItem(_ value: Any) -> [MCChatSegment]? {
    if let string = value as? String { return chatSegments(from: string) }
    guard JSONSerialization.isValidJSONObject(value),
          let data = try? JSONSerialization.data(withJSONObject: value),
          let json = String(data: data, encoding: .utf8) else { return nil }
    return chatSegments(from: json)
}

private func parseHoverItemObject(_ obj: [String: Any]) -> MCChatHoverItem? {
    let rawID = (obj["id"] as? String) ?? (obj["item"] as? String) ?? "minecraft:unknown"
    let resourceName = rawID.replacingOccurrences(of: "minecraft:", with: "")
    let count: Int = {
        if let n = obj["count"] as? NSNumber { return max(1, n.intValue) }
        if let n = obj["Count"] as? NSNumber { return max(1, n.intValue) }
        if let s = obj["Count"] as? String { return max(1, Int(s) ?? 1) }
        return 1
    }()
    let damage = Int16((obj["Damage"] as? NSNumber)?.intValue ?? (obj["damage"] as? NSNumber)?.intValue ?? 0)
    let legacy = legacyItemID(for: resourceName)
    var displayName = friendlyItemName(resourceName)
    ...
    if let components = obj["components"] as? [String: Any], enchantments.isEmpty {
        enchantments = parseEnchantments(components["minecraft:enchantments"])
    }
    if let tag = obj["tag"] as? [String: Any],
       let display = tag["display"] as? [String: Any] {
        if let name = display["Name"] as? String {
            displayName = name.replacingOccurrences(of: "§", with: "§")
                .split(separator: "\n", omittingEmptySubsequences: false).first.map(String.init) ?? name
            if let nameData = name.data(using: .utf8),
               let json = try? JSONSerialization.jsonObject(with: nameData),
               let segments = parseChatAnyForItem(json) {
                displayName = segments.map(\.text).joined()
            }
        }
        if let loreArray = display["Lore"] as? [Any] {
            lore = loreArray.compactMap { value in
                if let line = value as? String { return chatSegments(from: line) }
                return parseChatAnyForItem(value)
            }
        }
    }
...
private func parseLegacyHoverItemString(_ raw: String) -> MCChatHoverItem? {
    // Examples: {id:"minecraft:gold_block",Count:64b,tag:{...}}
    let idPattern = #"id\\s*[:=]\\s*[\\\"']?([A-Za-z0-9_:\-.]+)"#
    guard let idRegex = try? NSRegularExpression(pattern: idPattern),
          let match = idRegex.firstMatch(in: raw, range: NSRange(raw.startIndex..., in: raw)),
          let idRange = Range(match.range(at: 1), in: raw) else { return nil }
    let resourceName = String(raw[idRange]).replacingOccurrences(of: "minecraft:", with: "")
    let countPattern = #"(?:Count|count)\\s*[:=]\\s*(-?\\d+)"#
    var count = 1
    if let r = try? NSRegularExpression(pattern: countPattern),
       let m = r.firstMatch(in: raw, range: NSRange(raw.startIndex..., in: raw)),
       let cr = Range(m.range(at: 1), in: raw) {
        count = max(1, Int(raw[cr]) ?? 1)
    }
    let damagePattern = #"(?:Damage|damage)\\s*[:=]\\s*(-?\\d+)"#
    var damage: Int16 = 0
    if let r = try? NSRegularExpression(pattern: damagePattern),
       let m = r.firstMatch(in: raw, range: NSRange(raw.startIndex..., in: raw)),
       let dr = Range(m.range(at: 1), in: raw) {
        damage = Int16(Int(raw[dr]) ?? 0)
    }
    var displayName = friendlyItemName(resourceName)
    var lore: [[MCChatSegment]] = []
    var enchantments: [String] = []
    if let nameRegex = try? NSRegularExpression(pattern: #"(?:Name)\s*[:=]\s*\"((?:\\.|[^\"])*)\""#),
       let m = nameRegex.firstMatch(in: raw, range: NSRange(raw.startIndex..., in: raw)),
       let r = Range(m.range(at: 1), in: raw) {
        displayName = String(raw[r])
    }
    if let loreRegex = try? NSRegularExpression(pattern: #"Lore\s*[:=]\s*\[([^\]]*)\]"#),
       let m = loreRegex.firstMatch(in: raw, range: NSRange(raw.startIndex..., in: raw)),
       let r = Range(m.range(at: 1), in: raw) {
        let body = String(raw[r])
        if let lineRegex = try? NSRegularExpression(pattern: #"\"((?:\\.|[^\"])*)\""#) {
            for line in lineRegex.matches(in: body, range: NSRange(body.startIndex..., in: body)) {
                if let lr = Range(line.range(at: 1), in: body) {
                    lore.append(chatSegments(from: String(body[lr])))
                }
            }
        }
    }
    // NBT string dạng 1.12: Enchantments:[{id:"minecraft:sharpness",lvl:5s}, ...]
    if let enchRegex = try? NSRegularExpression(pattern: #"Enchantments\s*[:=]\s*\[([^\]]*)\]"#),
       let em = enchRegex.firstMatch(in: raw, range: NSRange(raw.startIndex..., in: raw)),
       let er = Range(em.range(at: 1), in: raw) {
        let body = String(raw[er])
        if let one = try? NSRegularExpression(pattern: #"id\s*[:=]\s*[\"']?([A-Za-z0-9_:\-.]+)[\"']?[^{}]*?lvl\s*[:=]\s*(-?\d+)"#) {
            for m in one.matches(in: body, range: NSRange(body.startIndex..., in: body)) {
                guard let ir = Range(m.range(at: 1), in: body), let lr = Range(m.range(at: 2), in: body) else { continue }
                let id = String(body[ir]).replacingOccurrences(of: "minecraft:", with: "")
                let lvl = Int(body[lr]) ?? 1
                enchantments.append("\(friendlyEnchantmentName(id)) \(romanNumeral(lvl))")
            }
        }
    }
    return MCChatHoverItem(resourceName: "minecraft:\(resourceName)", displayName: displayName,
                           count: count, legacyItemId: legacyItemID(for: resourceName), damage: damage,
                           lore: lore, enchantments: enchantments, rawValue: raw)
}

private func parseEnchantments(_ value: Any?) -> [String] {
    guard let arr = value as? [Any] else { return [] }
    var result: [String] = []
    for element in arr {
        if let dict = element as? [String: Any] {
            let rawIDString = (dict["id"] as? String) ?? (dict["Id"] as? String)
            let rawIDNumber: Int? = (dict["id"] as? NSNumber)?.intValue ?? (dict["Id"] as? NSNumber)?.intValue
            let rawID = rawIDString ?? rawIDNumber.map(String.init) ?? ""
            let rawLevel: Int = {
                if let n = dict["lvl"] as? NSNumber { return n.intValue }
                if let n = dict["level"] as? NSNumber { return n.intValue }
                if let s = dict["lvl"] as? String { return Int(s) ?? 1 }
                return 1
            }()
            let id = rawID.replacingOccurrences(of: "minecraft:", with: "")
            guard !id.isEmpty else { continue }
            result.append("\(friendlyEnchantmentName(id)) \(romanNumeral(rawLevel))")
        }
    }
    return result
}

private func friendlyEnchantmentName(_ id: String) -> String {
    let numeric: [String: String] = [
        ...
        "8":"Depth Strider", "9":"Frost Walker", "10":"Curse Of Binding", "11":"Soul Speed",
        "16":"Sharpness", "17":"Smite", "18":"Bane Of Arthropods", "19":"Knockback",
        "20":"Fire Aspect", "21":"Looting", "22":"Sweeping Edge", "32":"Efficiency",
        "33":"Silk Touch", "34":"Unbreaking", "35":"Fortune", "36":"Power",
        "48":"Power", "49":"Punch", "50":"Flame", "51":"Infinity",
        "61":"Luck Of The Sea", "62":"Lure", "70":"Mending"
    ]
    if let mapped = numeric[id] { return mapped }
    let key = id.replacingOccurrences(of: "_", with: " ")
...
private func friendlyItemName(_ resourceName: String) -> String {
    ...
    // Một số plugin gửi token kiểu item.appleGold.name thay vì resource id chuẩn.
    if let re = try? NSRegularExpression(pattern: "([a-z0-9])([A-Z])") {
        let range = NSRange(key.startIndex..., in: key)
        key = re.stringByReplacingMatches(in: key, options: [], range: range, withTemplate: "$1 $2")
    }
    return key.split(separator: " ").map { $0.prefix(1).uppercased() + $0.dropFirst() }.joined(separator: " ")
}

private func legacyItemID(for resourceName: String) -> Int16? {
    let table: [String: Int16] = [
        "air": 0, "stone": 1, "grass": 2, "dirt": 3, "cobblestone": 4, "planks": 5,
        "gold_block": 41, "iron_block": 42, "diamond_block": 57, "emerald_block": 133,
        "gold_ingot": 266, "iron_ingot": 265, "diamond": 264, "emerald": 388,
        "diamond_sword": 276, "diamond_pickaxe": 278, "diamond_axe": 279, "diamond_shovel": 277,
        "iron_sword": 267, "iron_pickaxe": 257, "iron_axe": 258, "iron_shovel": 256,
        "golden_sword": 283, "golden_pickaxe": 285, "golden_axe": 286, "golden_shovel": 284,
        "bow": 261, "arrow": 262, "compass": 345, "paper": 339, "book": 340,
        "map": 358, "bread": 297, "apple": 260, "golden_apple": 322,
        "fishing_rod": 346
    ]
    return table[resourceName.lowercased()]
}
...
private func zlibDeflate(_ data: Data) -> Data? {
    ...
    // IMPORTANT: Apple's COMPRESSION_ZLIB encoder produces *raw DEFLATE*
    // (RFC 1951), while the Minecraft protocol requires a complete zlib
    // stream (RFC 1950) when the packet's Data Length is non-zero.
    // Sending the raw bytes directly makes Bungee/Velocity throw errors such
    // as `DecoderException ... Unknown zlib block type` after a few packets.
    let capacity = max(data.count + 128, data.count * 2)
    var deflated = Data(count: capacity)
    let compressedSize: Int = deflated.withUnsafeMutableBytes { dstBuf in
    ...
    deflated = Data(deflated.prefix(compressedSize))

    // RFC 1950 header: CM=8 (DEFLATE), CINFO=7 (32 KiB window), FCHECK
    // chosen so the 16-bit header is divisible by 31.  0x78 0x9C is the
    // standard level-5/6 compatible header and is accepted by Java zlib.
    var result = Data([0x78, 0x9C])
    result.append(deflated)

    // RFC 1950 requires Adler-32 of the original (uncompressed) data in
    // network byte order at the end of the stream.
    var s1: UInt32 = 1
    var s2: UInt32 = 0
    ...
    let adler = (s2 << 16) | s1
    result.append(UInt8((adler >> 24) & 0xFF))
    result.append(UInt8((adler >> 16) & 0xFF))
    result.append(UInt8((adler >> 8) & 0xFF))
    result.append(UInt8(adler & 0xFF))
    return result
}
