...
final class MCClient: ObservableObject {
    ...
    /// Đảm bảo Client Settings + Plugin Message "MC|Brand" chỉ gửi đúng 1 lần, đúng lúc
    /// (ngay sau khi nhận Join Game — không phải ngay khi Login Success), giống hệt thứ
    /// tự 1 client Minecraft thật gửi. Reset lại mỗi lần connect/disconnect.
    private var hasSentJoinAnnouncement = false
    ...
    /// `autoLogin`: từ MCServerProfile.autoLogin — bật thì vào server xong tự "/dn" + tự
    ...
    /// Bắt đầu tick "còn sống" mỗi 3 giây — gửi lại đúng toạ độ/onGround hiện tại qua
    /// Player Position (0x0C) kể cả khi không có ai bấm WASD. Đây là hành vi bình
    /// thường của mọi client Minecraft thật (luôn gửi packet vị trí mỗi tick), không
    /// làm lệch vị trí thật vì toạ độ gửi lại y nguyên.
    ...
    private func handleLoginPacket(id: Int32, body: Data) {
        ...
        case 0x02: // Login Success -> chuyển sang Play
            ...
            state = .connected
            lastPacketReceivedAt = Date()
            // KHÔNG gửi Client Settings/Plugin Message ở đây. Login Success chỉ mới báo
            // "chuyển sang Play state" — server/proxy (đặc biệt proxy đa-version) còn cần
            // gửi Join Game (0x23) rồi mới thật sự sẵn sàng nhận packet Play từ client.
            // Gửi sớm hơn Join Game làm lệch thứ tự packet phía proxy -> desync -> không
            // vào được server. Client thật luôn đợi Join Game xong mới gửi 2 gói này (xem
            // handleJoinGame bên dưới).
            startWatchdog()
            startIdleAliveTicker()
            if UIApplication.shared.applicationState == .background {
                BackgroundKeepAlive.shared.start()
            }
            // attemptAutoLogin() cũng dời sang handleJoinGame() (cùng lý do thứ tự packet
            // ở trên) — không gọi ở đây nữa để không gửi /dn trước cả khi vào world thật sự.
    ...
    private func handleJoinGame(_ body: Data) {
        // Đúng thứ tự client thật: chỉ gửi Client Settings (0x04) + Plugin Message
        // "MC|Brand" (0x09) SAU KHI đã nhận Join Game — không sớm hơn. Bước này tách khỏi
        // phần parse dimension bên dưới có chủ đích: dù parse dimension có lỡ thất bại (vd
        // proxy trả body hơi khác) thì 2 gói bắt buộc này vẫn PHẢI được gửi, nếu không sẽ
        // quay lại đúng lỗi cũ (bị nghi bot vì thiếu tín hiệu client thật). Client thật/
        // TLauncher luôn gửi 2 gói này ngay sau Join Game, trước khi làm bất kỳ hành động
        // nào khác (chat, di chuyển...).
    ...
    func sendChat(_ text: String) {
        ...
        if let password = loginPassword(fromCommand: trimmed) {
            ...
            appendLog(.info, "Đã lưu mật khẩu đăng nhập cho tài khoản này — lần sau vào server sẽ tự động gửi /dn.")
            let generation = UUID()
            loginAutomationGeneration = generation
            let token = connectAttemptToken
