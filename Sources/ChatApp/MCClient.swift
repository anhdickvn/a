...
final class MCClient: ObservableObject {
    ...
    private func scheduleReconnect(reason: String) {
        ...
        // Disconnect = kết thúc phiên chat hiện tại. Lịch sử vẫn nằm trong Logs.
        log.removeAll()
        reconnectWorkItem?.cancel()
        connection?.cancel()
        connection = nil
        ...
        let work = DispatchWorkItem { [weak self] in
            ...
            Task { @MainActor in
                ...
                self.appendLog(.info, "Đang kết nối lại trực tiếp tới \(self.host):\(self.port) (protocol 760)...")
    ...
    private func handlePlayPacket(id: Int32, body: Data) {
        switch id {
        case 0x21: // Keep Alive (clientbound) — protocol 760 / 1.19.2
            guard body.count >= 8 else { return }
            let longBytes = Array(body.prefix(8))
            send(packetId: 0x10, payload: longBytes) // Keep Alive (serverbound), protocol 760

        case 0x0E: // Tab Complete (clientbound) — protocol 340
            handleTabComplete(body)

        case 0x34: // Player Info — protocol 760 / 1.19.2
            handlePlayerListItem(body)

        case 0x26: // Join Game — protocol 760
            handleJoinGame(body)

        case 0x3B: // Respawn — protocol 760
            handleRespawn(body)

        case 0x36: // Player Position And Look — protocol 760
        ...
        case 0x11: // Confirm Transaction (clientbound) — phải ACK click GUI trong protocol 340
            // Server gửi Window ID + Action Number + accepted. Client phải trả lại
            // cùng Window ID/Action Number/accepted; bỏ qua packet này có thể khiến
            // server khoá inventory interaction hoặc cuối cùng timeout.
            guard body.count >= 4 else { return }
            let windowID = body[body.startIndex]
            let actionHigh = body[body.startIndex + 1]
            let actionLow = body[body.startIndex + 2]
            let accepted = body[body.startIndex + 3]
            send(packetId: 0x05, payload: [windowID, actionHigh, actionLow, accepted])
        ...
        case 0x3A: // Resource Pack Send — protocol 760
            handleResourcePackSend(body)

        case 0x49: // Update Health — protocol 760
    ...
    func respawn() {
        ...
        send(packetId: 0x02, payload: MCVarInt.encode(0))
    ...
    private func sendPlayerPosition(onGround: Bool) {
        ...
        send(packetId: 0x12, payload: payload)
    }
    ...
    private func finishResourcePack(success: Bool, message: String) {
        ...
        send(packetId: 0x1F, payload: MCVarInt.encode(success ? 0 : 2))
    ...
    private func sendLoginAndScheduleMenu(password: String, token: UUID, generation: UUID) {
        guard useLoginForSession else { return }
        send(packetId: 0x03, payload: mcEncodeString("/login \(password)"))
    ...
    func useHotbarItem(_ hotbarIndex: Int) {
        ...
        send(packetId: 0x23, payload: [UInt8(hotbarIndex >> 8), UInt8(hotbarIndex & 0xFF)]) // Held Item Change (Short)
        send(packetId: 0x2C, payload: MCVarInt.encode(0)) // Use Item, hand = 0 (main hand), protocol 760
    ...
    func sendChat(_ text: String) {
        ...
        if let password = loginPassword(fromCommand: trimmed) {
            ...
            loginAutomationGeneration = generation
            let token = connectAttemptToken
            send(packetId: 0x03, payload: mcEncodeString(text))
        ...
        send(packetId: 0x03, payload: mcEncodeString(text)) // Chat (serverbound)
