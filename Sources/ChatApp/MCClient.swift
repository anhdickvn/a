...
final class MCClient: ObservableObject {
    ...
    private func logMovementContext(_ input: String) {
        ...
        movementDebug("PROTOCOL version=\(protocolVersion) fallback=\(fallbackProtocolVersion)", force: true)
        ...
        movementDebug("CANDIDATE movement IDs: 0x0D=Player Position, 0x0E=Player Position+Look, 0x0F=Player Look, 0x10=Player", force: true)
    }
    ...
    func movePlayer(dx: Double, dz: Double) {
        ...
        // Send Position+Look so every movement update carries the current view angles,
        // matching the packet shape used by the normal Minecraft client path.
        sendPlayerPositionAndLook(onGround: isOnGround)
        movementDebug(String(format: "MOVE TX \(movementPacketName(0x0E)): (%.3f,%.3f)->(%.3f,%.3f) step=%.3f onGround=%@",
