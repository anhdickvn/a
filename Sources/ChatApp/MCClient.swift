...
final class MCClient: ObservableObject {
    ...
    private func setupBackgroundObservers() {
        ...
        backgroundObservers.append(nc.addObserver(forName: UIApplication.willEnterForegroundNotification, object: nil, queue: .main) { [weak self] _ in
            Task { @MainActor [weak self] in
                ...
                // Chỉ xác định socket đã chết sau khi quay lại foreground.
                // Không coi việc vừa rời app là disconnect tức thời.
                ...
                if self.pendingBackgroundNetworkFailure != nil || self.connection == nil ||
                    (self.state == .connected && (backgroundDuration >= 10 && silentFor >= 10)) {
                    let reason = self.pendingBackgroundNetworkFailure ?? "socket không còn khả dụng sau khi app chạy nền"
                    self.pendingBackgroundNetworkFailure = nil
                    self.appendLog(.error, "Lỗi kết nối: The operation couldn’t be completed. (Network.NWError error 89 - Operation canceled by system/peer). \(reason)")
                    self.scheduleReconnect(reason: "socket stale sau khi quay lại app")
    ...
    private func handlePlayerPositionAndLook(_ body: Data) {
        ...
        movementSuppressedUntil = Date().timeIntervalSince1970 + 0.12
    ...
    /// One-shot micro movement used only to satisfy servers that unlock chat after
    /// the client has actually moved. One tap = one packet; there is no repeat task.
    func movePlayer(dx: Double, dz: Double) {
        ...
        guard now >= movementSuppressedUntil else {
            movementDebug("MOVE BLOCKED: waiting for server correction/teleport confirm", force: true)
            return
        }
        let distance = hypot(dx, dz)
        guard distance > 0.0001 else { return }

        // Conservative one-tap probe. The goal is only to make the server observe
        // a legitimate PlayerMoveEvent so chat-unlock plugins stop replying
        // "You cannot chat until you move!". Do not run a background movement loop.
        let step: Double = 0.06
        let scale = min(step / distance, 1.0)
        let oldX = playerX
        let oldZ = playerZ
        playerX += dx * scale
        playerZ += dz * scale
        ...
        movementLastSentAt = Date().timeIntervalSince1970
        movementDebug(String(format: "MICRO MOVE PRIMARY: (%.3f, %.3f) -> (%.3f, %.3f), step=%.3f", oldX, oldZ, playerX, playerZ, step), force: true)

        // Primary form: Player Position (0x0D).
        sendPlayerPosition(onGround: isOnGround)

        // Compatibility fallback: if the server did not issue a correction,
        // send one Player Position And Look (0x0E) for the same server position.
        // This is still finite: at most 2 movement packets per tap.
        movementProbeTask = Task { @MainActor [weak self] in
            ...
            self.movementDebug("MICRO MOVE FALLBACK: 0x0E position+look", force: true)
            self.sendPlayerPositionAndLook(onGround: self.isOnGround)
    ...
    /// One-shot jump: send a single small upward position change, then a single
    /// landing packet. This is intentionally not a client-side physics loop.
    func jumpPlayer() {
        guard state == .connected else {
        ...
        guard isOnGround else {
            movementDebug("JUMP BLOCKED: onGround=false", force: true)
            return
        }

        // A tap on Space is a single airborne position update. Do not create a
        // local physics loop; the server remains authoritative and can correct us.
        let startY = playerY
        isOnGround = false
        playerY = startY + 0.12
        movementSuppressedUntil = Date().timeIntervalSince1970 + 0.18
        movementDebug(String(format: "JUMP ONE-SHOT: Y %.3f -> %.3f", startY, playerY), force: true)
