...
final class MCClient: ObservableObject {
    ...
    private func handleConfirmTransaction(_ body: Data) {
        ...
        payload.append(acceptedByte == 0 ? 0 : 1)

        let acceptedText = acceptedByte == 0 ? "false" : "true"
        let note = "confirm window=\(windowID.0) action=\(actionNumber) accepted=\(acceptedText)"
        dbg("RX Confirm Transaction: \(note) → trả ACK 0x05")
