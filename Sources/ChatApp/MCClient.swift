...
final class MCClient: ObservableObject {
        ...
        backgroundObservers.append(nc.addObserver(forName: UIApplication.willEnterForegroundNotification, object: nil, queue: .main) { [weak self] _ in
            Task { @MainActor [weak self] in
                ...
                if self.pendingBackgroundNetworkFailure != nil || self.connection == nil ||
                    (self.state == .connected && (backgroundDuration >= 15 && silentFor >= 15)) {
    ...
    private func send(packetId: Int32, payload: [UInt8]) {
        ...
        if isMovementPacket && movementDebugEnabled {
            ...
            movementDebug("WIRE TX \(String(format: "0x%02X", packetId)) seq=\(movementSequence) bytes=\(payload.count) connection=\(connection == nil ? "nil" : "present")", force: true)
        }
    ...
    private func handlePlayerPositionAndLook(_ body: Data) {
        ...
        movementDebug(String(format: "SERVER CORRECTION: x=%.3f y=%.3f z=%.3f yaw=%.1f pitch=%.1f flags=0x%02X teleport=%d", playerX, playerY, playerZ, playerYaw, playerPitch, flagByte, teleport.0), force: movementDebugEnabled)
    ...
    func movePlayer(dx: Double, dz: Double) {
        ...
        movementDebug(String(format: "MOVE TX (%.3f,%.3f)->(%.3f,%.3f) step=%.3f onGround=%@",
