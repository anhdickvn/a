...
final class MCClient: ObservableObject {
    ...
    /// One-shot micro movement. A single tap sends a tiny step immediately and
    /// one optional second tick shortly afterwards. There is intentionally no
    /// repeating WASD controller; this is only to satisfy servers that require
    /// a real movement before chat is unlocked.
    ...
    /// Calculate one movement vector from the current server yaw and send it once.
    func movePlayer(forward: Double, strafe: Double) {
        ...
        let generation = microMovementGeneration &+ 1
        microMovementGeneration = generation
        ...
        // One additional tiny tick mimics a short real touch without leaving a
        // background controller running. Any server correction invalidates it.
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.06) { [weak self] in
            guard let self,
                  self.state == .connected,
                  self.hasServerPosition,
                  self.microMovementGeneration == generation else { return }
            self.playerX += dx * 0.045
            self.playerZ += dz * 0.045
            self.movementLastSentAt = Date().timeIntervalSince1970
            self.sendPlayerPosition(onGround: self.isOnGround)
            self.movementDebug(String(format: "MICRO MOVE STEP 2: x=%.3f z=%.3f",
                                       self.playerX, self.playerZ), force: true)
        }
    }
    ...
    /// Existing UIKit callbacks are kept, but the touch is now one-shot.
    ...
    func stopMoving(forward: Double, strafe: Double) {
        // No repeated task exists anymore; keep callback for the existing UIKit API.
