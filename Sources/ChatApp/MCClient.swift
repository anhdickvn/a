...
final class MCClient: ObservableObject {
    ...
    private func startIdleAliveTicker() {
        ...
        idleAliveTask = Task { @MainActor [weak self] in
            while !Task.isCancelled {
                ...
                if self.hasServerPosition {
                    self.sendPlayerPosition(onGround: self.isOnGround)
                } else {
                    self.sendPlayerGroundStatus(onGround: self.isOnGround)
                }
            }
        }
    }
    ...
    func movePlayer(dx: Double, dz: Double) {
        ...
        // One deliberate step only. Large enough to be observable by typical
        // server-side movement listeners, but sent exactly once per tap.
        let step: Double = 0.20
        ...
        movementDebug(String(format: "MICRO MOVE: (%.3f, %.3f) -> (%.3f, %.3f), onGround=%@",
                             oldX, oldZ, playerX, playerZ,
