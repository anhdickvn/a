...
final class MCClient: ObservableObject {
    ...
    private func handlePlayerPositionAndLook(_ body: Data) {
        ...
        jumpTask = nil
        jumpGeneration += 1
        movementLastSentAt = Date().timeIntervalSince1970 + 0.10
    ...
    func movePlayer(dx: Double, dz: Double) {
        ...
        // A small but meaningful client movement. 0.12 blocks is large enough to
        // produce a real server-side move event while staying far below sprint speed.
        let step: Double = 0.12
    ...
    func movePlayer(forward: Double, strafe: Double) {
        ...
        movementDebug("INPUT \(movementName(forward: forward, strafe: strafe)): ONE-SHOT", force: true)
    ...
    func jumpPlayer() {
        ...
        // Give the server one short interval to observe the jump, then land once.
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.12) { [weak self] in
            ...
            self.movementDebug(String(format: "JUMP LAND: Y=%.3f", self.playerY), force: true)
        }
    }
