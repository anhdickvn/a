...
final class MCClient: ObservableObject {
    ...
    private let supportedProtocolRange: ClosedRange<Int32> = 338...340 // logic packet hiện tại dùng schema tương thích này
...
private func zlibInflate(_ data: Data, expectedSize: Int) -> Data? {
    // ChatCraft/logic cũ dùng COMPRESSION_ZLIB cho phần raw DEFLATE sau
    // khi bỏ header/trailer của zlib. Một số proxy gửi stream theo dạng này;
    // truyền nguyên RFC1950 vào compression_decode_buffer khiến packet hợp lệ
    // bị coi là "nén không hợp lệ".
    guard expectedSize > 0, data.count > 6 else { return nil }
    ...
    let resultSize: Int = result.withUnsafeMutableBytes { dst -> Int in
        raw.withUnsafeBytes { src -> Int in
            ...
            return compression_decode_buffer(dstPtr, expectedSize, srcPtr, raw.count, nil, COMPRESSION_ZLIB)
        }
    }
    guard resultSize == expectedSize else { return nil }
    return result
}
