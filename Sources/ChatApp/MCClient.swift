...
final class MCClient: ObservableObject {
    ...
    private func send(packetId: Int32, payload: [UInt8]) {
        ...
        recordProtocolTrace(direction: "TX", packetId: packetId, bytes: wirePayloadLength, detail: packetTraceDetail(direction: "TX", packetId: packetId, payload: payload))
        ...
        connection?.send(content: Data(full), completion: .contentProcessed { [weak self] error in
            guard let self else { return }
            if error == nil, packetId == 0x0B {
                Task { @MainActor in
                    // Chỉ coi KeepAlive là đã được queue thành công khi NWConnection
                    // báo contentProcessed. Nếu send fail, pending ID được giữ lại để
                    // snapshot cuối phiên chỉ rõ vấn đề.
                    self.pendingKeepAliveID = nil
                    self.lastKeepAliveResponseAt = Date()
                }
            }
            ...
            Task { @MainActor in
                ...
                if packetId == 0x0B {
                    self.appendLog(.error, "🔴 KeepAlive response 0x0B gửi lỗi: \(error.localizedDescription)")
                }
        ...
        connection?.receive(minimumIncompleteLength: 1, maximumLength: 65536) { [weak self] data, _, isComplete, error in
            ...
            Task { @MainActor in
                ...
                if let error {
                    ...
                    self.dumpProtocolTrace(reason: "NWConnection receive error: \(error.localizedDescription)")
                    if self.shouldStayConnected {
                ...
                if isComplete {
                    if self.state != .disconnected {
                        ...
                        self.dumpProtocolTrace(reason: "TCP EOF — server chủ động đóng socket")
                        if self.shouldStayConnected {
    ...
    private func handleRawPacket(_ raw: Data) async {
        ...
        lastReceivedPacketAt = Date()
        recordProtocolTrace(direction: "RX", packetId: packetId, bytes: body.count, detail: packetTraceDetail(direction: "RX", packetId: packetId, payload: Array(body)))
    ...
    private func handleLoginPacket(id: Int32, body: Data) {
        ...
        case 0x02: // Login Success -> chuyển sang Play
            ...
            resetProtocolTrace()
            dbg("Login Success — vào Play. Bắt đầu debug trace wire 80 packet gần nhất.")
            startWatchdog()
    ...
    private func handlePlayPacket(id: Int32, body: Data) {
        ...
        case 0x1F: // Keep Alive (clientbound) — 8 byte long, phải gửi lại y nguyên
            guard body.count >= 8 else {
                dbg("⚠️ RX KeepAlive 0x1F nhưng body chỉ có \(body.count) byte (cần 8).")
                return
            }
            let longBytes = Array(body.prefix(8))
            let keepAliveID = Int64(bitPattern: longBytes.reduce(UInt64(0)) { ($0 << 8) | UInt64($1) })
            lastKeepAliveReceivedAt = Date()
            pendingKeepAliveID = keepAliveID
            pendingKeepAliveReceivedAt = Date()
            dbg("RX KeepAlive id=\(keepAliveID). Trả lời ngay bằng 0x0B.")
            send(packetId: 0x0B, payload: longBytes) // Keep Alive (serverbound)
            lastKeepAliveResponseAt = Date()
        ...
        case 0x1A: // Disconnect (play)
            ...
            dumpProtocolTrace(reason: "Server gửi Disconnect (0x1A): \(readableReason)")
        ...
        default:
            unknownPlayPacketCount += 1
            dbg("⚠️ RX packet Play chưa được xử lý: id=0x\(String(format: "%02X", id)) bytes=\(body.count) (unknown #\(unknownPlayPacketCount)).")
        }
    }
    ...
    private func handlePlayerPositionAndLook(_ body: Data) {
        ...
        lastTeleportID = teleport.0
        lastTeleportReceivedAt = Date()
        dbg("RX Player Position And Look teleportId=\(teleport.0), flags=0x\(String(format: "%02X", flagByte)). Gửi Teleport Confirm 0x00.")
        send(packetId: 0x00, payload: MCVarInt.encode(teleport.0))
        lastTeleportConfirmAt = Date()
    }
    ...
    // MARK: - Protocol wire trace (v27)
    // Ring-buffer nhỏ để khi socket bị server đóng, ta biết chính xác các packet
    // cuối cùng đã RX/TX thay vì chỉ nhìn "packet cuối đã gửi". Không thay đổi
    // protocol behaviour; chỉ quan sát wire traffic.
    private var protocolTrace: [String] = []
    private let protocolTraceLimit = 80
    private var lastReceivedPacketID: Int32?
    private var lastReceivedPacketAt: Date = .distantPast
    private var lastKeepAliveReceivedAt: Date = .distantPast
    private var lastKeepAliveResponseAt: Date = .distantPast
    private var pendingKeepAliveID: Int64?
    private var pendingKeepAliveReceivedAt: Date = .distantPast
    private var lastTeleportID: Int32?
    private var lastTeleportReceivedAt: Date = .distantPast
    private var lastTeleportConfirmAt: Date = .distantPast
    private var unknownPlayPacketCount = 0

    private func resetProtocolTrace() {
        protocolTrace.removeAll(keepingCapacity: true)
        lastReceivedPacketID = nil
        lastReceivedPacketAt = .distantPast
        lastKeepAliveReceivedAt = .distantPast
        lastKeepAliveResponseAt = .distantPast
        pendingKeepAliveID = nil
        pendingKeepAliveReceivedAt = .distantPast
        lastTeleportID = nil
        lastTeleportReceivedAt = .distantPast
        lastTeleportConfirmAt = .distantPast
        unknownPlayPacketCount = 0
    }

    private func protocolElapsed() -> String {
        guard let start = debugSessionStart else { return "T+?" }
        return String(format: "T+%.2fs", Date().timeIntervalSince(start))
    }

    private func packetTraceDetail(direction: String, packetId: Int32, payload: [UInt8]) -> String {
        switch (direction, packetId) {
        case ("RX", 0x1F):
            if payload.count >= 8 {
                let value = Int64(bitPattern: payload.prefix(8).reduce(UInt64(0)) { ($0 << 8) | UInt64($1) })
                return "KeepAlive id=\(value)"
            }
            return "KeepAlive malformed"
        case ("TX", 0x0B):
            if payload.count >= 8 {
                let value = Int64(bitPattern: payload.prefix(8).reduce(UInt64(0)) { ($0 << 8) | UInt64($1) })
                return "KeepAlive response id=\(value)"
            }
            return "KeepAlive response malformed"
        case ("RX", 0x2F):
            return "Player Position And Look (server correction)"
        case ("TX", 0x00) where state == .connected:
            return "Teleport Confirm / packet 0x00"
        case ("TX", 0x0C):
            return "Player / On Ground=\(payload.first == 1)"
        case ("TX", 0x0D):
            return "Player Position xyz present, onGround=\(payload.last == 1)"
        case ("TX", 0x04):
            return "Client Settings"
        case ("TX", 0x09):
            return "Plugin Message MC|Brand"
        case ("TX", 0x18):
    ...
    private func recordProtocolTrace(direction: String, packetId: Int32, bytes: Int, detail: String) {
        let idText = String(format: "0x%02X", packetId)
        let suffix = detail.isEmpty ? "" : " · \(detail)"
        let line = "\(protocolElapsed()) \(direction) \(idText) bytes=\(bytes)\(suffix)"
    ...
    private func dumpProtocolTrace(reason: String) {
        let now = Date()
        let sinceRx = lastReceivedPacketAt == .distantPast ? -1 : now.timeIntervalSince(lastReceivedPacketAt)
        let sinceKeepAlive = lastKeepAliveReceivedAt == .distantPast ? -1 : now.timeIntervalSince(lastKeepAliveReceivedAt)
        let sinceKeepAliveReply = lastKeepAliveResponseAt == .distantPast ? -1 : now.timeIntervalSince(lastKeepAliveResponseAt)
        let sinceTeleport = lastTeleportReceivedAt == .distantPast ? -1 : now.timeIntervalSince(lastTeleportReceivedAt)
        let sinceTeleportConfirm = lastTeleportConfirmAt == .distantPast ? -1 : now.timeIntervalSince(lastTeleportConfirmAt)

        var diagnosis = "⚠️ [PROTOCOL DIAGNOSTIC] \(reason)\n"
        diagnosis += "· state=\(state), protocol=\(protocolVersion) (\(versionHint(for: protocolVersion)))\n"
        diagnosis += "· last RX=\(lastReceivedPacketID.map { String(format: "0x%02X", $0) } ?? "none"), cách=\(sinceRx >= 0 ? String(format: "%.2fs", sinceRx) : "n/a")\n"
        diagnosis += "· KeepAlive RX cách=\(sinceKeepAlive >= 0 ? String(format: "%.2fs", sinceKeepAlive) : "n/a"), reply cách=\(sinceKeepAliveReply >= 0 ? String(format: "%.2fs", sinceKeepAliveReply) : "n/a"), pending=\(pendingKeepAliveID.map(String.init) ?? "none")\n"
        diagnosis += "· Teleport RX id=\(lastTeleportID.map(String.init) ?? "none"), cách=\(sinceTeleport >= 0 ? String(format: "%.2fs", sinceTeleport) : "n/a"), confirm cách=\(sinceTeleportConfirm >= 0 ? String(format: "%.2fs", sinceTeleportConfirm) : "n/a")\n"
        diagnosis += "· last TX=\(lastSentPacketDescription)\n"
        diagnosis += "· hasServerPosition=\(hasServerPosition), currentWindow=\(currentWindow == nil ? "nil" : "open"), unknownPlayPackets=\(unknownPlayPacketCount)\n"

        if pendingKeepAliveID != nil {
            diagnosis += "· 🔴 NGHI NGỜ: KeepAlive đã nhận nhưng chưa xác nhận response hoàn tất.\n"
        } else if lastKeepAliveReceivedAt == .distantPast {
            diagnosis += "· 🟠 CHƯA THẤY KeepAlive nào trong trace — cần kiểm tra server/proxy timeout hoặc protocol.\n"
        } else if sinceKeepAlive > 30 {
            diagnosis += "· 🟠 Server im lặng lâu sau KeepAlive gần nhất; cần xem 80 packet cuối bên dưới.\n"
        } else {
            diagnosis += "· 🟢 KeepAlive đã được RX/TX trong phiên; lỗi không còn giống KeepAlive timeout đơn giản.\n"
        }

        if lastTeleportID != nil && sinceTeleportConfirm < 0.0 {
            diagnosis += "· 🔴 NGHI NGỜ: có Teleport nhưng chưa thấy Teleport Confirm.\n"
        }

        diagnosis += "\n--- 80 PACKET GẦN NHẤT ---\n"
        diagnosis += protocolTrace.joined(separator: "\n")
        appendLog(.debug, diagnosis, segments: [MCChatSegment(text: diagnosis, colorHex: "#F2B84B")])
    }
