...
final class MCClient: ObservableObject {
        ...
        idleAliveTask = Task { @MainActor [weak self] in
            while !Task.isCancelled {
                ...
                if self.hasServerPosition {
                    self.sendIdlePositionAndLook(onGround: self.isOnGround)
                } else {
    ...
    private func handlePlayerPositionAndLook(_ body: Data) {
        ...
        if !hasServerPosition {
            dbg("Nhận Player Position And Look ĐẦU TIÊN từ server (x=\(String(format: "%.1f", playerX)), y=\(String(format: "%.1f", playerY)), z=\(String(format: "%.1f", playerZ)), teleportId=\(teleport.0)). Từ giờ heartbeat chuyển sang gửi 0x0F kèm rung yaw/pitch nhẹ (giả lập người thật).")
        }
    ...
    /// Pha dao động dùng để tạo rung yaw/pitch giả lập bàn tay/chuột người thật khi
    /// đứng yên. Tăng dần mỗi lần idle ticker chạy (~1 lần/giây).
    private var idleLookPhase: Double = 0

    /// FIX (v27): root cause thật của "This command cannot be executed" xuất hiện sau
    /// ~60-65s dù đứng yên, bất kể có GUI hay không, bất kể heartbeat 0x0D vẫn đều đặn:
    /// app CHƯA BAO GIỜ gửi packet có yaw/pitch (Player Look 0x0E / Player Position And
    /// Look 0x0F) — chỉ gửi Player Position (0x0D, không có góc nhìn). Với server thật,
    /// player đứng yên tuyệt đối, KHÔNG BAO GIỜ xoay đầu dù chỉ 1 độ, trong khi client
    /// Minecraft thật (kể cả TLauncher) luôn có rung tay/chuột rất nhỏ liên tục — anti-bot
    /// phổ biến nhất dùng chính tín hiệu "0 độ xoay đầu trong ~60 giây" để phân biệt bot,
    /// và mốc quan sát được (64.57s / 64.59s, gần như giống hệt nhau ở 2 lần test dù thao
    /// tác khác nhau) khớp chính xác với 1 ngưỡng thời gian cố định phía server, không
    /// liên quan gì đến GUI hay heartbeat X/Y/Z.
    ///
    /// Gửi 0x0F (Player Position And Look) thay vì 0x0D khi idle, với yaw/pitch dao động
    /// nhẹ (biên độ ~1-1.5 độ, dạng sin/cos mượt) quanh góc nhìn hiện tại — KHÔNG ghi đè
    /// `playerYaw`/`playerPitch` thật (những giá trị này dùng để tính hướng di chuyển
    /// WASD), chỉ cộng tạm vào lúc build payload gửi đi.
    private func sendIdlePositionAndLook(onGround: Bool) {
        idleLookPhase += 0.35
        let jitterYaw = Float(sin(idleLookPhase) * 1.2)
        let jitterPitch = Float(cos(idleLookPhase * 0.73) * 0.6)
        var payload: [UInt8] = []
        payload += playerX.mcBigEndianBytes
        payload += playerY.mcBigEndianBytes
        payload += playerZ.mcBigEndianBytes
        payload += (playerYaw + jitterYaw).mcBigEndianBytes
        payload += (playerPitch + jitterPitch).mcBigEndianBytes
        payload.append(onGround ? 1 : 0)
        // Protocol 340: Player Position And Look (serverbound) = 0x0F.
        send(packetId: 0x0F, payload: payload)
    }
