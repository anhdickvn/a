...
final class MCClient: ObservableObject {
    ...
    // v27 protocol diagnostic ring buffer: giữ lại packet wire-level gần nhất để
    // khi socket tự đóng có thể xác định chính xác server/client đang trao đổi gì.
    // Không đưa từng packet lên UI vì chunk/world traffic có thể rất lớn.
    private var protocolTrace: [String] = []
    private let protocolTraceLimit = 80
    private var lastRXPacketDescription = "none"
    private var lastTXPacketDescription = "none"
    private var lastKeepAliveID: Int64?
    private var lastKeepAliveRXAt: Date?
    private var lastKeepAliveTXAt: Date?
    private var lastTeleportID: Int32?
    private var lastTeleportRXAt: Date?
    private var lastConfirmTransactionWindow: UInt8?
    private var lastConfirmTransactionAction: Int16?
    private var lastConfirmTransactionAccepted: Bool?
    private var lastConfirmTransactionRXAt: Date?
    private var lastConfirmTransactionTXAt: Date?
    ...
    // MARK: - v27 protocol diagnostic

    private func resetProtocolTrace() {
        protocolTrace.removeAll(keepingCapacity: true)
        lastRXPacketDescription = "none"
        lastTXPacketDescription = "none"
        lastKeepAliveID = nil
        lastKeepAliveRXAt = nil
        lastKeepAliveTXAt = nil
        lastTeleportID = nil
        lastTeleportRXAt = nil
        lastConfirmTransactionWindow = nil
        lastConfirmTransactionAction = nil
        lastConfirmTransactionAccepted = nil
        lastConfirmTransactionRXAt = nil
        lastConfirmTransactionTXAt = nil
    ...
    private func tracePacket(direction: String, id: Int32, payloadBytes: Int, note: String = "") {
        ...
        if let start = debugSessionStart {
            elapsed = String(format: "T+%.2fs", Date().timeIntervalSince(start))
        } else {
            elapsed = "T+?"
        }
        let stateName: String
        switch state {
        case .loggingIn: stateName = "LOGIN"
        case .connected: stateName = "PLAY"
        case .connecting: stateName = "CONNECTING"
        case .disconnected: stateName = "DISCONNECTED"
        case .failed: stateName = "FAILED"
        }
        let suffix = note.isEmpty ? "" : " | " + note
        let line = "[\(elapsed)] \(direction) \(stateName) id=0x\(String(format: "%02X", id)) bytes=\(payloadBytes)\(suffix)"
        protocolTrace.append(line)
        if protocolTrace.count > protocolTraceLimit {
            protocolTrace.removeFirst(protocolTrace.count - protocolTraceLimit)
        }
        if direction == "RX" {
            lastRXPacketDescription = line
        } else {
            lastTXPacketDescription = line
        }
    }
    ...
    private func protocolPacketName(_ id: Int32, direction: String) -> String {
        if direction == "RX" {
            switch id {
            case 0x00: return "Teleport? / Login Disconnect"
            case 0x0E: return "Tab Complete"
            case 0x0F: return "Chat Message"
            case 0x11: return "Confirm Transaction"
            case 0x1A: return "Disconnect"
            case 0x1F: return "KeepAlive"
            case 0x23: return "Join Game"
            case 0x2E: return "Player List Item"
            case 0x2F: return "Player Position And Look"
            case 0x35: return "Respawn"
            case 0x12: return "Close Window"
            case 0x13: return "Open Window"
            case 0x14: return "Window Items"
            case 0x16: return "Set Slot"
            case 0x41: return "Update Health"
            default: return "unknown"
            }
        } else {
            switch id {
            case 0x00: return "Teleport Confirm / Login Start"
            case 0x03: return "Client Status"
            case 0x04: return "Client Settings"
            case 0x05: return "Confirm Transaction"
            case 0x07: return "Click Window"
            case 0x09: return "Plugin Message"
            case 0x0B: return "KeepAlive"
            case 0x0C: return "Player Ground"
            case 0x0D: return "Player Position"
            case 0x01: return "Chat Message"
            default: return "unknown"
            }
        }
    }
    ...
    private func dumpProtocolTrace(trigger: String) {
        var lines: [String] = []
        lines.append("⚠️ [PROTOCOL DIAGNOSTIC] \(trigger)")
        if let start = debugSessionStart {
            lines.append(String(format: "• Session age: %.2fs", Date().timeIntervalSince(start)))
        }
        lines.append("• state=\(state)")
        lines.append("• protocol=\(protocolVersion) (\(versionHint(for: protocolVersion)))")
        lines.append("• compressionThreshold=\(compressionThreshold)")
        lines.append("• hasServerPosition=\(hasServerPosition), currentWindow=\(currentWindow.map { "id=\($0.windowId),slots=\($0.slotCount)" } ?? "nil")")
        lines.append("• last RX: \(lastRXPacketDescription)")
        lines.append("• last TX: \(lastTXPacketDescription)")
        if let keepAlive = lastKeepAliveID {
            let rxAge = lastKeepAliveRXAt.map { String(format: "%.2fs", Date().timeIntervalSince($0)) } ?? "n/a"
            let txAge = lastKeepAliveTXAt.map { String(format: "%.2fs", Date().timeIntervalSince($0)) } ?? "n/a"
            lines.append("• KeepAlive id=\(keepAlive), lastRXAge=\(rxAge), lastTXAge=\(txAge)")
        } else {
            lines.append("• KeepAlive: chưa thấy trong phiên")
        }
        if let teleport = lastTeleportID {
            let age = lastTeleportRXAt.map { String(format: "%.2fs", Date().timeIntervalSince($0)) } ?? "n/a"
            lines.append("• TeleportConfirm id=\(teleport), RXAge=\(age)")
        } else {
            lines.append("• Teleport: chưa thấy trong phiên")
        }
        if let window = lastConfirmTransactionWindow,
           let action = lastConfirmTransactionAction,
           let accepted = lastConfirmTransactionAccepted {
            let rxAge = lastConfirmTransactionRXAt.map { String(format: "%.2fs", Date().timeIntervalSince($0)) } ?? "n/a"
            let txAge = lastConfirmTransactionTXAt.map { String(format: "%.2fs", Date().timeIntervalSince($0)) } ?? "n/a"
            lines.append("• ConfirmTransaction window=\(window), action=\(action), accepted=\(accepted), RXAge=\(rxAge), TXAge=\(txAge)")
        } else {
            lines.append("• ConfirmTransaction: chưa thấy trong phiên")
        }
        if let reason = lastDisconnectReason {
            lines.append("• Disconnect reason: \(reason)")
        }
        lines.append("• ---- last \(protocolTrace.count) packets ----")
        lines.append(contentsOf: protocolTrace)
        for line in lines {
            dbg(line)
        }
    }
    ...
    private func send(packetId: Int32, payload: [UInt8]) {
        ...
        var note = protocolPacketName(packetId, direction: "TX")
        if packetId == 0x0B, payload.count >= 8 {
            let value = Int64(bitPattern: payload.prefix(8).reduce(UInt64(0)) { ($0 << 8) | UInt64($1) })
            lastKeepAliveID = value
            lastKeepAliveTXAt = Date()
            note += " keepAliveId=\(value)"
        }
        tracePacket(direction: "TX", id: packetId, payloadBytes: payload.count, note: note)
        let full = MCVarInt.encode(Int32(body.count)) + body
        if packetId == 0x0D || packetId == 0x0C {
            movementDebug("WIRE TX 0x\(String(format: "%02X", packetId)) bytes=\(payload.count) connection=\(connection == nil ? "nil" : "present")", force: movementDebugEnabled)
        }
    ...
    private func handleRawPacket(_ raw: Data) async {
        ...
        var note = protocolPacketName(packetId, direction: "RX")
        if packetId == 0x1F, body.count >= 8 {
            let value = Int64(bitPattern: body.prefix(8).reduce(UInt64(0)) { ($0 << 8) | UInt64($1) })
            lastKeepAliveID = value
            lastKeepAliveRXAt = Date()
            note += " keepAliveId=\(value)"
        } else if packetId == 0x2F {
            if let teleport = decodeVarInt(from: Data(body), offset: 33)?.0 {
                lastTeleportID = teleport
                lastTeleportRXAt = Date()
                note += " teleportId=\(teleport)"
            }
        }
        tracePacket(direction: "RX", id: packetId, payloadBytes: body.count, note: note)
    ...
    private func handlePlayPacket(id: Int32, body: Data) {
        ...
        case 0x1A: // Disconnect (play)
            ...
            lastDisconnectReason = readableReason
    ...
    private func handleConfirmTransaction(_ body: Data) {
        ...
        lastConfirmTransactionWindow = windowID.0
        lastConfirmTransactionAction = actionNumber
        lastConfirmTransactionAccepted = acceptedByte != 0
        lastConfirmTransactionRXAt = Date()
        ...
        lastConfirmTransactionTXAt = Date()
    ...
    private func movementDebug(_ text: String, force: Bool = false) {
        ...
        movementDebugLines.append(line)
        if movementDebugLines.count > 300 {
            movementDebugLines.removeFirst(movementDebugLines.count - 300)
        }
    }
    ...
    /// Một bước movement thật theo hướng nhìn server.
    func movePlayer(dx: Double, dz: Double) {
        ...
        let now = Date().timeIntervalSince1970
        guard now - movementLastSentAt >= 0.045 else { return }
        let distance = hypot(dx, dz)
        guard distance > 0 else {
            movementDebug("MOVE BLOCKED: zero vector")
            return
        }
        ...
        if movementDebugEnabled && now - lastMovementDebugAt >= 0.08 {
            ...
            movementDebug(String(format: "MOVE TX: (%.3f, %.3f) -> (%.3f, %.3f), yaw=%.1f, onGround=%@", oldX, oldZ, playerX, playerZ, playerYaw, isOnGround ? "true" : "false"))
        }
    }
    ...
    /// Dedicated 20 Hz controller: gộp các phím đang giữ, tránh 2 Task chạy đè nhau.
    func startMoving(forward: Double, strafe: Double) {
        guard state == .connected else {
            movementDebug("TOUCH DOWN \(movementName(forward: forward, strafe: strafe)) nhưng state=\(state)")
            return
        }
        guard hasServerPosition else {
            movementDebug("TOUCH DOWN \(movementName(forward: forward, strafe: strafe)) nhưng chưa có server position")
            return
        }
        ...
        if movementControlTask == nil {
            movementControlTask = Task { @MainActor [weak self] in
                while !Task.isCancelled {
                    ...
                    if self.activeMovementInputs.isEmpty { continue }
                    let forward = self.activeMovementInputs.values.reduce(0) { $0 + $1.forward }
                    let strafe = self.activeMovementInputs.values.reduce(0) { $0 + $1.strafe }
    ...
    func jumpPlayer() {
        ...
        movementDebug(String(format: "JUMP DOWN: startY=%.3f", playerY), force: true)
        jumpTask?.cancel()
        jumpGeneration += 1
    ...
    /// Ghi 1 dòng debug trace hiển thị NGAY trong khung chat (màu xám, tiền tố 🔧),
    /// kèm mốc thời gian tương đối so với lúc Login Success — để khi chụp màn hình,
    /// người xem thấy được đúng trình tự & khoảng cách thời gian giữa các bước auto-login,
    /// GUI, và lúc lỗi "This command cannot be executed" xuất hiện.
    private func dbg(_ text: String) {
        ...
        let line = "🔧 [\(elapsed)] \(text)"
        appendLog(.debug, line, segments: [MCChatSegment(text: line, colorHex: "#9AA0A6")])
    }

    /// Snapshot đầy đủ trạng thái client, gọi ngay khi phát hiện dấu hiệu lỗi
    /// "This command cannot be executed" (chat message hoặc disconnect reason) — gom hết
    /// thông tin cần để chẩn đoán vào 1 khối duy nhất, nằm ngay dưới dòng lỗi trong chat.
    private func dumpDiagnosticSnapshot(trigger: String) {
        var lines: [String] = []
        lines.append("⚠️ [DIAGNOSTIC] Phát hiện: \(trigger)")
        if let start = debugSessionStart {
            lines.append("· Thời gian từ Login Success: \(String(format: "%.2fs", Date().timeIntervalSince(start)))")
        }
        lines.append("· hasServerPosition = \(hasServerPosition)")
        lines.append("· state = \(state)")
        if let window = currentWindow {
            let hasItem23 = window.items[23] != nil
            lines.append("· currentWindow: windowId=\(window.windowId), slotCount=\(window.slotCount), slot23=\(hasItem23 ? (window.items[23]?.plainName ?? "?") : "trống")")
        } else {
            lines.append("· currentWindow = nil (không có GUI nào đang mở)")
        }
        lines.append("· Giây kể từ packet cuối nhận được: \(String(format: "%.2fs", Date().timeIntervalSince(lastPacketReceivedAt)))")
        lines.append("· Giây kể từ lần gửi vị trí/heartbeat cuối: \(String(format: "%.2fs", Date().timeIntervalSince1970 - movementLastSentAt))")
        lines.append("· Packet gần nhất đã gửi: \(lastSentPacketDescription)")
        let text = lines.joined(separator: "\n")
        appendLog(.debug, text, segments: [MCChatSegment(text: text, colorHex: "#F2B84B")])
    }
}
