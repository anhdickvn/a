...
final class MCClient: ObservableObject {
    ...
    func connect(host: String, port: UInt16, username: String, useLogin: Bool = false) {
        ...
        // Dò protocol thật của server trước khi login. Một số proxy/server (như
        // proxy.toichoi.com) sẽ CHỦ ĐỘNG kick với lý do "Outdated client/server"
        // nếu handshake gửi sai protocol version — cứ hardcode 760 mà server không
        // chấp nhận là sẽ bị đá ngay lập tức, không thể vào được.
        probeProtocolVersion(host: host, port: port) { [weak self] probed in
            guard let self, self.connectAttemptToken == token else { return }
            if let probed {
                self.protocolVersion = probed
                self.appendLog(.info, "Server báo protocol \(probed) (\(self.versionHint(for: probed))). Đang kết nối với protocol này...")
            } else {
                self.protocolVersion = self.fallbackProtocolVersion
                self.appendLog(.info, "Không dò được protocol server, dùng mặc định \(self.fallbackProtocolVersion)...")
            }
        ...
        let work = DispatchWorkItem { [weak self] in
            ...
            Task { @MainActor in
                ...
                self.probeProtocolVersion(host: self.host, port: self.port) { [weak self] probed in
                    guard let self, self.connectAttemptToken == token else { return }
                    self.protocolVersion = probed ?? self.fallbackProtocolVersion
                    self.openMainConnection(token: token)
                }
            }
        }
