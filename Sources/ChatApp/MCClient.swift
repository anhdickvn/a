...
final class MCClient: ObservableObject {
    ...
    private func setupBackgroundObservers() {
        ...
        backgroundObservers.append(nc.addObserver(forName: UIApplication.willEnterForegroundNotification, object: nil, queue: .main) { [weak self] _ in
            Task { @MainActor [weak self] in
                ...
                if self.shouldStayConnected && self.state == .disconnected {
            ...
            if dataLen == 0 {
                ...
                guard let inflated = zlibInflate(rest, expectedSize: expected) else {
                    ...
                    if now - lastCompressionFailureAt >= 2.0 {
                        ...
                        appendLog(.error, "Packet nén không hợp lệ (dataLength=\(expected), compressed=\(rest.count)). Bỏ qua packet, giữ đồng bộ TCP.")
                    }
    ...
    func clickWindowSlot(_ slot: Int, mouseButton: UInt8 = 0) {
        ...
        // Trường Clicked Item là item đang cầm bằng con trỏ, KHÔNG phải item ở ô vừa click.
        // App không có cursor-item riêng nên trạng thái đúng khi chọn menu là ô trống: Short(-1).
        // Gửi lại raw item của chính ô đang click có thể làm plugin/server nhận giao dịch bất thường.
        let itemBytes: [UInt8] = [0xFF, 0xFF]
    ...
    func useHotbarItem(_ hotbarIndex: Int) {
        ...
        send(packetId: 0x09, payload: [UInt8(hotbarIndex >> 8), UInt8(hotbarIndex & 0xFF)]) // Held Item Change (Short)
...
private func zlibInflate(_ data: Data, expectedSize: Int) -> Data? {
    ...
    // `expectedSize` comes directly from Minecraft's Data Length VarInt.
    // COMPRESSION_ZLIB handles the complete RFC1950 stream (header + deflate + checksum).
    // Dùng đúng kích thước server khai báo trước; nếu stream không khớp thì coi packet
    // là lỗi và bỏ qua, không bao giờ cố đọc tiếp từ dữ liệu nén.
...
private func zlibDeflate(_ data: Data) -> Data? {
    ...
    // Dữ liệu Minecraft có thể gần như không nén được. Buffer cũ quá nhỏ sẽ làm
    // compression_encode_buffer trả 0, nhưng code cũ vẫn gắn Data Length > 0 và
    // gửi dữ liệu RAW khiến phía server giải nén thất bại. Dùng buffer dư rộng để
    // không bao giờ đánh dấu packet là compressed nếu thực tế chưa nén thành công.
    var capacity = max(256, data.count + data.count / 2 + 128)
    for _ in 0..<3 {
    ...
    // Caller sẽ vẫn đưa packet qua đường uncompressed khi threshold không đạt;
    // tuyệt đối không giả vờ đây là zlib data.
    return nil
}
