...
final class MCClient: ObservableObject {
    ...
    private func send(packetId: Int32, payload: [UInt8]) {
        ...
        appendNetworkDebug(String(format: "TX state=%@ packet=0x%02X payload=%d wire=%d %@", String(describing: state), packetId, payload.count, wirePayloadLength, compressed ? "compressed" : "raw"))
    ...
    private func handleRawPacket(_ raw: Data) async {
        ...
        appendNetworkDebug(String(format: "RX state=%@ packet=0x%02X body=%d wire=%d %@", String(describing: state), packetId, body.count, raw.count, compressionThreshold >= 0 ? "compression=ON" : "compression=OFF"))
    ...
    /// Debug mạng được đưa thẳng vào chat để người dùng có thể lưu cùng ChatLogs.
    /// Không ghi payload/credential nhạy cảm; chỉ ghi state, packet ID, kích thước và thời gian.
    private func appendNetworkDebug(_ text: String) {
        appendLog(.debug, "[DEBUG] \(text)")
    }
}
