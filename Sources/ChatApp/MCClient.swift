...
private func zlibDeflate(_ data: Data) -> Data? {
    ...
    // Apple COMPRESSION_ZLIB deliberately emits RAW DEFLATE (RFC1951), while
    // Minecraft/Netty/Bungee's protocol compression expects a zlib-wrapped stream
    // (RFC1950). The old implementation sent Apple's raw stream directly; Bungee
    // then reported DecoderException / Unknown zlib header after the client had
    // been connected for a while and a compressed packet was sent.
    let capacity = max(data.count * 2 + 128, data.count + 256)
    var raw = Data(count: capacity)
    let compressedSize: Int = raw.withUnsafeMutableBytes { dstBuf in
    ...
    let rawDeflate = raw.prefix(compressedSize)
    var wrapped = Data([0x78, 0x9C]) // CMF/FLG, zlib header for deflate level 5/default.
    wrapped.append(rawDeflate)

    // Adler-32 of the UNCOMPRESSED packet, big-endian, required by RFC1950.
    var a: UInt32 = 1
    var b: UInt32 = 0
    for byte in data {
