...
final class MCClient: ObservableObject {
        ...
        if compressionThreshold >= 0 {
            ...
            if body.count >= Int(compressionThreshold) {
                if let compressed = zlibDeflate(Data(body)) {
                    // Minecraft/Netty/Bungee expects RFC1950 zlib framing around the
                    // DEFLATE stream. Apple's Compression framework's COMPRESSION_ZLIB
                    // encoder is used here only for the DEFLATE payload; explicitly add
                    // the zlib header + Adler-32 trailer so the wire format is unambiguous.
                    let zlibStream = makeZlibStream(rawDeflate: compressed, uncompressed: Data(body))
                    let dataLen = MCVarInt.encode(Int32(body.count))
                    body = dataLen + Array(zlibStream)
                } else {
    ...
    func jumpPlayer() {
        guard state == .connected, hasServerPosition, isOnGround else { return }
        jumpTask?.cancel()
        jumpGeneration += 1
...
/// Build a standards-compliant RFC1950 zlib stream from the raw DEFLATE payload
/// produced by the Compression framework. Minecraft protocol compression uses
/// zlib framing (CMF/FLG + DEFLATE + Adler-32), not a bare RFC1951 stream.
private func makeZlibStream(rawDeflate: Data, uncompressed: Data) -> Data {
    var out = Data([0x78, 0x9C]) // CM=8, CINFO=7, FLEVEL=2; header checksum divisible by 31
    out.append(rawDeflate)

    // Adler-32, RFC1950 section 2.2, stored big-endian.
    var s1: UInt32 = 1
    var s2: UInt32 = 0
