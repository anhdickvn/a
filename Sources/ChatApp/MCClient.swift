...
final class MCClient: ObservableObject {
    ...
    /// Đồng bộ lại cờ Autologin cho phiên đang chạy. Cần thiết cho trường hợp người dùng
    /// vẫn đang "Stay connected" (chưa disconnect), vào Edit server đổi toggle Autologin,
    /// rồi quay lại màn chat: vì `isConnectedTo(...)` đã true nên `connect()` KHÔNG được gọi
    /// lại, và nếu không có hàm này thì `autoLoginEnabled` vẫn giữ giá trị CŨ (vd vẫn true dù
    /// người dùng vừa tắt) — app tiếp tục tự gửi /dn + tự mở la bàn, chèn vào lúc người dùng
    /// đang cố tự gõ lệnh bằng tay, khiến thao tác thủ công trông như "không ăn".
    func syncAutoLogin(_ autoLogin: Bool) {
    ...
    /// Vanilla không spam movement packet 20 lần/giây khi đứng yên.
    /// Khi không đổi vị trí/góc nhìn, Player (0x0C) được gửi theo nhịp
    /// khoảng 20 ticks (xấp xỉ 1 giây). Gửi 20 lần/giây có thể bị proxy/plugin
    /// coi là packet flood/automation và là nguyên nhân rất đáng ngờ của lỗi
    /// "This command cannot be executed on this server" xuất hiện sau khi idle.
    ...
    private func startIdleAliveTicker() {
        ...
        idleAliveTask = Task { @MainActor [weak self] in
            while !Task.isCancelled {
                ...
                guard now - self.movementLastSentAt >= 1.5 else { continue }
    ...
    private func handlePlayPacket(id: Int32, body: Data) {
        ...
        case 0x1A: // Disconnect (play)
            ...
            // Server-issued Disconnect là quyết định của proxy/plugin. Không tự
            // reconnect ngay lập tức vì điều đó tạo vòng lặp kick -> reconnect -> kick
            // và có thể khiến người dùng tưởng client đang spam command. Người dùng
            // vẫn có thể bấm Connect lại từ danh sách server.
            shouldStayConnected = false
            reconnectWorkItem?.cancel()
            reconnectWorkItem = nil
            state = .disconnected
    ...
    /// LƯU Ý: trước đây hàm này bắt buộc `hasServerPosition == true` (tức là phải nhận được
    /// gói Player Position And Look 0x2F từ server trước). Một số proxy/plugin không luôn
    /// gửi gói đó ngay, nên `hasServerPosition` có thể không bao giờ thành true và WASD bị
    /// khoá vĩnh viễn dù đã kết nối thành công. Giờ chỉ cần `state == .connected`: nếu chưa
    /// có toạ độ thật từ server, ta di chuyển từ gốc (0,0,0) tạm thời — server sẽ tự gửi
    /// Player Position And Look để sửa lại toạ độ đúng ngay khi nhận được gói di chuyển đầu
    /// tiên, giống hệt cách client Minecraft thật xử lý phần đầu phiên chơi.
