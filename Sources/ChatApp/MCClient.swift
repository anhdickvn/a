...
final class MCClient: ObservableObject {
    ...
    private func handlePlayerPositionAndLook(_ body: Data) {
        ...
        movementControlTask?.cancel()
        movementControlTask = nil
        for task in movementBurstTasks.values { task.cancel() }
        movementBurstTasks.removeAll()
        activeMovementInputs.removeAll()
        jumpTask?.cancel()
        jumpTask = nil
        jumpGeneration += 1
    ...
    func movePlayer(dx: Double, dz: Double) {
        ...
        // Giữ movement ở Player Position (0x0D). Không dùng 0x0E cho WASD ở đây:
        // khi server/proxy đang correction, việc liên tục gửi Position+Look làm vòng
        // correction diễn ra dày đặc và có thể bị disconnect. Góc nhìn chỉ gửi khi
        // thực sự đổi ở phần look riêng.
    ...
    /// Dedicated 20 Hz controller: gộp các phím đang giữ.
    /// Đồng thời hỗ trợ tap ngắn: một lần chạm WASD vẫn tạo ra một burst movement
    /// ngắn thay vì phụ thuộc hoàn toàn vào việc giữ ngón tay.
        ...
        if movementControlTask == nil {
            movementControlTask = Task { @MainActor [weak self] in
                while !Task.isCancelled {
                    ...
                    guard self.state == .connected, self.hasServerPosition else { continue }
    ...
    func stopMoving(forward: Double, strafe: Double) {
        ...
        // Tap-friendly fallback: if the user released almost immediately, keep a very short
        // movement burst so a normal tap still has a visible/gameplay effect. Holding the button
        // keeps using the 20 Hz controller above.
        if activeMovementInputs.isEmpty {
            ...
            movementBurstTasks[key] = Task { @MainActor [weak self] in
                ...
                if self.movementBurstTasks[key] != nil { self.movementBurstTasks[key] = nil }
        ...
        if !activeMovementInputs.isEmpty { return }
    }
    ...
    private func sendPlayerPositionAndLook(onGround: Bool) {
        ...
        payload.append(onGround ? 1 : 0)
        // Protocol 340: Player Position And Look (serverbound) = 0x0E.
