...
final class MCClient: ObservableObject {
    ...
    func disconnect(silent: Bool = false) {
        ...
        stopWatchdog()
        stopIdleAliveTicker()
        activeMovementInputs.removeAll()
    ...
    private func jumpPlayerTick(generation: Int, startY: Double, startTime: TimeInterval) {
        ...
        let dy = velocity * elapsed - 0.5 * gravity * elapsed * elapsed
        playerY = startY + 0.42 + dy
        movementLastSentAt = Date().timeIntervalSince1970
        sendPlayerPosition(onGround: false)
