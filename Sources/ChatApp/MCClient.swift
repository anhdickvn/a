...
final class MCClient: ObservableObject {
    ...
    private func handleResourcePackSend(_ body: Data) {
        ...
        appendLog(.system, "Server gửi texture pack, bắt buộc tải và áp dụng...")
    ...
    private func downloadResourcePack(urlString: String, hash: String) {
        ...
        request.timeoutInterval = 20 // Tránh treo âm thầm nếu URL chậm/không phản hồi —
                                      // nhiều server có giới hạn thời gian chờ tải texture
                                      // pack, treo lâu dễ bị coi là client không phản hồi.
    ...
    private func finishResourcePack(success: Bool, message: String) {
        resourcePackPending = false
        appendLog(success ? .system : .error, message)
