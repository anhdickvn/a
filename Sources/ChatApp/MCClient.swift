...
final class MCClient: ObservableObject {
    ...
    /// Short finite movement pulse. One tap creates three tiny vanilla-style position
    /// updates over ~120 ms, then stops. It is not a background movement loop.
    func movePlayer(dx: Double, dz: Double) {
        guard state == .connected else {
            movementDebug("MOVE BLOCKED: state=\(state)", force: true); return
        }
        guard hasServerPosition else {
            movementDebug("MOVE BLOCKED: hasServerPosition=false", force: true); return
        }
        let now = Date().timeIntervalSince1970
        guard now >= movementSuppressedUntil else {
            movementDebug("MOVE BLOCKED: waiting for server correction/teleport confirm", force: true); return
        }
        let d = hypot(dx, dz)
        guard d > 0.0001 else { return }
        movementProbeTask?.cancel()
        microMovementGeneration &+= 1
        let generation = microMovementGeneration
        let ux = dx / d, uz = dz / d
        let perTick = 0.06
        movementDebug(String(format: "MOVE PULSE START dir=(%.3f,%.3f) step=%.3f", ux, uz, perTick), force: true)
        movementProbeTask = Task { @MainActor [weak self] in
            guard let self else { return }
            for tick in 0..<3 {
                guard !Task.isCancelled, self.state == .connected, self.hasServerPosition, self.microMovementGeneration == generation else {
                    self.movementDebug("MOVE PULSE STOP tick=\(tick) state=\(self.state)", force: true)
                    self.movementProbeTask = nil
                    return
                }
                let oldX=self.playerX, oldZ=self.playerZ
                self.playerX += ux*perTick
                self.playerZ += uz*perTick
                self.displayedPlayerX=self.playerX
                self.displayedPlayerZ=self.playerZ
                self.movementLastSentAt = Date().timeIntervalSince1970
                self.movementDebug(String(format:"MOVE PULSE tick=%d %.3f,%.3f -> %.3f,%.3f", tick+1, oldX, oldZ, self.playerX, self.playerZ), force:true)
                self.sendPlayerPosition(onGround: self.isOnGround)
                if tick < 2 { try? await Task.sleep(nanoseconds: 60_000_000) }
            }
            self.movementDebug("MOVE PULSE END", force:true)
            self.movementProbeTask=nil
        }
    }

    /// Calculate one direction from current yaw and send exactly one micro-move packet.
    func movePlayer(forward: Double, strafe: Double) {
        ...
        movementDebug("INPUT \(movementName(forward: forward, strafe: strafe)): ONE-SHOT dx=\(String(format: "%.3f", dx)) dz=\(String(format: "%.3f", dz))", force: true)
    ...
    /// UIKit touch-down is intentionally one-shot. No repeat timer/task is created.
    func startMoving(forward: Double, strafe: Double) {
        movementDebug("TOUCH DOWN \(movementName(forward: forward, strafe: strafe)) -> ONE-SHOT", force: true)
        movePlayer(forward: forward, strafe: strafe)
    }

    func stopMoving(forward: Double, strafe: Double) {
        movementDebug("TOUCH UP \(movementName(forward: forward, strafe: strafe))", force: true)
    }

    /// One tap jump: one upward packet + one short authoritative resync.
    func jumpPlayer() {
        guard state == .connected else { movementDebug("JUMP BLOCKED: state=\(state)", force:true); return }
        guard hasServerPosition else { movementDebug("JUMP BLOCKED: hasServerPosition=false", force:true); return }
        guard isOnGround else { movementDebug("JUMP BLOCKED: onGround=false", force:true); return }
        let startY=playerY
        isOnGround=false
        playerY=startY+0.42
        displayedPlayerY=playerY
        movementSuppressedUntil=Date().timeIntervalSince1970+0.30
        movementDebug(String(format:"JUMP PULSE %.3f -> %.3f", startY, playerY), force:true)
        ...
        movementProbeTask = Task { @MainActor [weak self] in
            ...
            guard let self, !Task.isCancelled, self.state == .connected, self.hasServerPosition, self.microMovementGeneration == generation else { return }
            self.playerY=self.displayedPlayerY
            self.isOnGround=true
            self.movementDebug(String(format:"JUMP RESYNC Y=%.3f", self.playerY), force:true)
...
private func parseEnchantments(_ value: Any?) -> [String] {
    ...
    for element in arr {
        if let dict = element as? [String: Any] {
            let rawID = (dict["id"] as? String) ?? (dict["Id"] as? String) ?? ""
            let rawLevel: Int = {
