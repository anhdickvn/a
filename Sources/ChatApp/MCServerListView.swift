...
struct MCServerListView: View {
    ...
    private func serverRow(_ profile: MCServerProfile) -> some View {
        ...
        return HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                if let status {
                    switch status.phase {
                    case .online:
                        if !status.motdSegments.isEmpty {
                            serverMotdText(status.motdSegments)
                                .font(.system(size: 15, weight: .regular))
                                .lineLimit(2)
                                .frame(maxWidth: .infinity, alignment: .leading)
                        } else if !status.motd.isEmpty {
                            Text(status.motd)
                                .font(.system(size: 15))
                                .foregroundStyle(.primary)
                                .lineLimit(2)
        ...
        .background(Color(red: 0.10, green: 0.10, blue: 0.11), in: RoundedRectangle(cornerRadius: 10))
        .contentShape(Rectangle())
    }

    @ViewBuilder
    private func serverMotdText(_ segments: [MCChatSegment]) -> some View {
        segments.reduce(Text("")) { partial, seg in
            var piece = Text(seg.text)
                .foregroundStyle(seg.colorHex.flatMap(Color.init(hex:)) ?? .primary)
            if seg.bold { piece = piece.bold() }
            if seg.italic { piece = piece.italic() }
            if seg.underline { piece = piece.underline() }
            if seg.strikethrough { piece = piece.strikethrough() }
