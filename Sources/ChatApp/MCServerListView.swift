...
struct MCServerListView: View {
                ...
                Section {
                    if accountStore.accounts.isEmpty {
                        ...
                        ForEach(accountStore.accounts) { account in
                            HStack(spacing: 12) {
                                MCUserHeadIcon(username: account.username, size: 52)
    ...
    private func serverRow(_ profile: MCServerProfile) -> some View {
        ...
        return HStack(alignment: .top, spacing: 12) {
            MCServerFavicon(status: status, size: 72)
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                if let account = accountStore.account(for: profile.accountId) {
                    HStack(spacing: 5) {
                        MCUserHeadIcon(username: account.username, size: 20)
...
private struct MCUserHeadIcon: View {
    let username: String
    let size: CGFloat

    var body: some View {
        AsyncImage(url: URL(string: "https://mc-heads.net/avatar/\(username)/\(Int(size))")) { phase in
            switch phase {
...
private struct MCServerFavicon: View {
    let status: MCServerStatus?
    let size: CGFloat

    var body: some View {
        Group {
            if let base64 = status?.faviconBase64,
