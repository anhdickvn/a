...
struct MCServerListView: View {
                ...
                Section {
                    if store.profiles.isEmpty {
                        ...
                        ForEach(store.profiles) { profile in
                            ...
                            // Trước đây hàng server chỉ có swipe-to-reveal (vuốt trái) để Sửa/Xóa,
                            // không có cách nào bấm chuột phải (trackpad/chuột rời trên iPad) hay
                            // giữ để mở menu — nên "chuột phải chọn máy chủ" không phản ứng gì.
                            // .contextMenu tự động xử lý CẢ press-and-hold (chạm) LẪN secondary
                            // click (chuột phải) trên cùng 1 hàng.
                            .contextMenu {
                                Button { editingProfile = profile } label: {
                                    Label("Sửa", systemImage: "pencil")
                                }
    ...
    private func accountHeadIcon(_ username: String) -> some View {
        AsyncImage(url: URL(string: "https://mc-heads.net/avatar/\(username)/72")) { phase in
    ...
    private func serverRow(_ profile: MCServerProfile) -> some View {
        ...
        return HStack(alignment: .top, spacing: 12) {
            ...
            // Nút "..." luôn hiện sẵn (không cần vuốt, không cần chuột phải/giữ) để Sửa/Xóa —
            // Menu ở đây tự mở như 1 popup, không kích hoạt NavigationLink của cả hàng.
            Menu {
                Button { editingProfile = profile } label: {
                    Label("Sửa", systemImage: "pencil")
                }
