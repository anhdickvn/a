...
final class MCProfileStore: ObservableObject {
    ...
    /// Đảm bảo server mẫu của Tôi Chơi Network luôn có sẵn trong danh sách.
    /// Không tạo trùng nếu người dùng đã có đúng host/port này.
    func ensureBuiltInToiChoiServer() {
    ...
    /// Giữ API cũ để project cũ không lỗi compile; không tự xóa server mẫu nữa.
    func removeLegacyInjectedServer() {
        ensureBuiltInToiChoiServer()
    }
}
